    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Gerencie fazendas, sítios e glebas do portfólio</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <Card key={stat.label} className="bg-zinc-950/50 border-zinc-800">
            <CardContent className="p-4">
              <p className={cn("text-2xl font-black", stat.color)}>{stat.value}</p>
              <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mt-1">{stat.label}</p>
            </CardContent>
          </Card>
        ))}
      </div>
            ...
            {[{ key: "all", label: "Todos" }, ...Object.entries(CAR_STATUS).map(([k, v]) => ({ key: k, label: v.label }))].map(f => (
              <button
                ...
                className={cn(
                  ...
                  filterStatus === f.key ? "bg-emerald-600 text-white" : "bg-zinc-900 text-zinc-400 hover:bg-slate-700"
                )}
              >
                {f.label}
              </button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <Card className="w-full max-w-xl bg-zinc-950 border-slate-700 shadow-2xl max-h-[90vh] overflow-y-auto">
            <CardHeader className="flex flex-row items-center justify-between sticky top-0 bg-zinc-950 z-10 pb-4 border-b border-zinc-800">
            ...
            <CardContent className="pt-5">
              <form onSubmit={handleSave} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {/* Proprietário */}
                  <div className="space-y-1 col-span-full">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Proprietário / Cliente</label>
                    <select
                      value={form.customer_id}
                      onChange={e => setForm({ ...form, customer_id: e.target.value })}
                      className="w-full h-10 rounded-xl bg-zinc-900 border border-slate-700 text-white px-3 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Área Total (ha)</label>
                    <Input
                      type="number"
                      step="0.01"
                      min="0"
                  ...
                  {/* Bioma */}
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Bioma</label>
                    <select
                      value={form.biome}
                  ...
                  {/* CAR */}
                  <div className="space-y-1 col-span-full">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Código CAR</label>
                    <Input
                  ...
                  {/* Status CAR */}
                  <div className="space-y-1 col-span-full">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Status CAR</label>
                  ...
                  {/* Observações */}
                  <div className="space-y-1 col-span-full">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Observações Internas</label>
                    <textarea
                      value={form.notes}
                      onChange={e => setForm({ ...form, notes: e.target.value })}
                      placeholder="Informações adicionais sobre a propriedade..."
                      rows={3}
                      className="w-full rounded-xl bg-zinc-900 border border-slate-700 text-white px-3 py-2.5 text-sm placeholder:text-zinc-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 resize-none"
                    />
                  </div>
                </div>
            ...
            return (
              <Card key={prop.id} className="bg-zinc-950/50 border-zinc-800 hover:border-slate-700 transition-colors">
                <CardContent className="p-5">
                  ...
                  {/* Details Grid */}
                  <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs text-zinc-500 mb-4">
                    {prop.total_area_ha && (
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-zinc-800 flex-wrap">
                    <Button
                      ...
                      className="flex-1 h-8 text-xs text-zinc-400 hover:text-white hover:bg-zinc-900 rounded-lg"
                    >
