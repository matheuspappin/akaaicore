...
export default function WhatsAppPage() {
  ...
  return (
    ...
    <div className="space-y-6">
        ...
        {[
          ...
          <Card key={item.title} className="bg-white/5 dark:bg-black/50 border border-white/10 dark:border-white/10 hover:shadow-md transition-shadow">
            <CardContent className="p-5">
              ...
              <h3 className="font-bold text-white dark:text-white mb-1">{item.title}</h3>
      ...
      <Card className="bg-gradient-to-br from-red- to-teal-900 border-0 text-white overflow-hidden">
      ...
      <Dialog open={isQrModalOpen} onOpenChange={setIsQrModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col items-center gap-4 py-4">
            {qrCode ? (
              <img src={qrCode} alt="QR Code WhatsApp" className="w-64 h-64 object-contain bg-white/5 p-2 rounded-lg" />
            ) : (
              <div className="w-64 h-64 flex items-center justify-center bg-slate-100 dark:bg-zinc-950 rounded-lg">
                <RefreshCw className="w-12 h-12 animate-spin text-zinc-400" />
              </div>
            )}
