...
const COLORS = ["border-l-red-500", "border-l-red-500", "border-l-red-", "border-l-red-", "border-l-red-"]
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link href="/solutions/estudio-de-danca/teacher/chamada">
          <Button className="bg-[#e40014] hover:bg-[#e40014] text-white font-bold rounded-xl shadow-lg shadow-red-600/20">
        ...
        {[
          ...
          <Card key={s.label} className="bg-white/5 bg-black border border-white/10 border-white/10">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xl font-black text-white dark:text-white">{s.value}</p>
              <p className="text-[10px] font-bold uppercase text-zinc-400 mt-0.5 leading-tight">{s.label}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card className="bg-white/5 bg-black border border-white/10 border-white/10">
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <Link href="/solutions/estudio-de-danca/teacher/turmas">
            <Button variant="ghost" size="sm" className="text-[#e40014] font-bold text-xs">
              ...
              <div key={turma.id} className={cn("p-4 rounded-xl bg-black dark:bg-white/5 border-l-4", COLORS[i % COLORS.length])}>
                <div className="flex items-center justify-between">
                  ...
                  <Link href={`/solutions/estudio-de-danca/teacher/chamada?classId=${turma.id}`}>
                    <Button size="sm" className="bg-[#e40014] hover:bg-[#e40014] text-white font-bold rounded-xl text-xs">
                      Chamada
                    </Button>
                  </Link>
                </div>
