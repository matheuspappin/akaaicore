...
export async function proxy(request: NextRequest) {
  ...
  const webAppBase = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  const redirectUrl = (path: string) =>
    isAdminApp && !path.startsWith('/admin')
      ? `${webAppBase}${path.startsWith('/') ? path : '/' + path}`
  ...
  // ─── Arquivos estáticos e rotas internas do Next.js ───────────────────────
    ...
    if (process.env.MAINTENANCE_MODE === 'true') {
      ...
      if (!isAdminPath && !isMaintenancePage) {
        ...
        return NextResponse.redirect(redirectUrl('/maintenance'))
      }
    }
    ...
    if (isVerticalizationProtected && activeVertical) {
      if (!user) {
        return NextResponse.redirect(redirectUrl(activeVertical.loginPath))
      }
      const role = userRole || ''
      ...
      if (pathname === `/solutions/${activeVertical.basePath}/dashboard`) {
        ...
        if (redirect) return NextResponse.redirect(redirectUrl(redirect))
    ...
    if (activeVertical && pathname === activeVertical.loginPath && user && userRole) {
      ...
      return NextResponse.redirect(redirectUrl(redirect))
    }
    ...
    if (user && effectiveRole) {
      if (isAuthRoute) {
        if (effectiveRole === 'seller') return NextResponse.redirect(redirectUrl('/seller'))
        if (effectiveRole === 'finance') return NextResponse.redirect(redirectUrl('/finance'))
        if (effectiveRole === 'affiliate' || effectiveRole === 'partner') return NextResponse.redirect(redirectUrl('/portal/affiliate/dashboard'))
        if (effectiveRole === 'engineer') return NextResponse.redirect(redirectUrl('/solutions/fire-protection/engineer'))
        if (effectiveRole === 'technician') return NextResponse.redirect(redirectUrl('/solutions/fire-protection/technician'))
        if (effectiveRole === 'teacher') return NextResponse.redirect(redirectUrl('/solutions/estudio-de-danca/teacher'))
        ...
        if (effectiveRole === 'student') {
          ...
          if (niche === 'fire_protection') return NextResponse.redirect(redirectUrl('/solutions/fire-protection/client'))
          if (niche === 'agroflowai' || niche === 'agro') return NextResponse.redirect(redirectUrl('/solutions/agroflowai/client'))
          // Default: portal genérico de aluno (dance/gym/outros)
          return NextResponse.redirect(redirectUrl('/student'))
        }
        return NextResponse.redirect(redirectUrl('/dashboard'))
      }

      if (isEngineerRoute && effectiveRole !== 'engineer' && effectiveRole !== 'super_admin') {
        return NextResponse.redirect(redirectUrl('/solutions/fire-protection/login'))
      }
      ...
      if (isTeacherRoute && effectiveRole !== 'teacher' && effectiveRole !== 'super_admin') {
        if (effectiveRole === 'engineer') return NextResponse.redirect(redirectUrl('/solutions/fire-protection/engineer'))
        return NextResponse.redirect(redirectUrl('/solutions/estudio-de-danca/login'))
      }
      ...
      if (isDashboardRoute && effectiveRole !== 'admin' && effectiveRole !== 'teacher' && effectiveRole !== 'finance' && effectiveRole !== 'super_admin') {
        ...
        if (effectiveRole === 'technician') return NextResponse.redirect(redirectUrl('/solutions/fire-protection/technician'))
        if (effectiveRole === 'teacher') return NextResponse.redirect(redirectUrl('/solutions/estudio-de-danca/teacher'))
        if (effectiveRole === 'student') return NextResponse.redirect(redirectUrl('/student'))
        return NextResponse.redirect(redirectUrl('/login'))
      }
      ...
      if ((pathname === '/portal/affiliate/login' || pathname === '/portal/affiliate/register') && (effectiveRole === 'affiliate' || effectiveRole === 'partner')) {
        return NextResponse.redirect(redirectUrl('/portal/affiliate/dashboard'))
      }
      ...
      if (isAffiliateRoute) {
        return NextResponse.redirect(redirectUrl('/portal/affiliate/login'))
      }
      return NextResponse.redirect(redirectUrl('/login'))
    }
