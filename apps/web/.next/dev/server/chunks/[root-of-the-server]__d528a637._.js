;!function(){try { var e="undefined"!=typeof globalThis?globalThis:"undefined"!=typeof global?global:"undefined"!=typeof window?window:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&((e._debugIds|| (e._debugIds={}))[n]="7f77effb-c9ee-f563-d4be-3cc6b2e011f2")}catch(e){}}();
module.exports = [
...
"[project]/apps/web/app/api/dance-studio/dashboard/stats/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
    ...
    try {
        ...
        const [studentsRes, teachersRes, classesRes, paymentsRes] = await Promise.all([
            ...
            supabase.from('professionals').select('id', {
                count: 'exact',
                head: true
            }).eq('studio_id', studioId).eq('professional_type', 'teacher').eq('status', 'active'),
            supabase.from('classes').select(`
          id, name, dance_style, schedule, status,
          professional:professionals(id, name)
        `).eq('studio_id', studioId).eq('status', 'active'),
            supabase.from('payments').select('amount, status, payment_date, due_date').eq('studio_id', studioId)
        ]);
        ...
        const monthPaid = payments.filter((p)=>p.status === 'paid' && (p.payment_date && p.payment_date.startsWith(currentMonth) || p.due_date && p.due_date.startsWith(currentMonth)));
        const faturamento = monthPaid.reduce((s, p)=>s + Number(p.amount || 0), 0);
        // Mês anterior para crescimento
        const prevMonthPaid = payments.filter((p)=>p.status === 'paid' && (p.payment_date && p.payment_date.startsWith(prevMonth) || p.due_date && p.due_date.startsWith(prevMonth)));
        const faturamentoAnterior = prevMonthPaid.reduce((s, p)=>s + Number(p.amount || 0), 0);
        const growthPercent = faturamentoAnterior > 0 ? Math.round((faturamento - faturamentoAnterior) / faturamentoAnterior * 100) : faturamento > 0 ? 100 : 0;
        // Turmas de hoje: classes com schedule contendo day_of_week = todayDow
        const turmasDeHoje = classes.filter((c)=>{
            const schedule = c.schedule ?? [];
            return schedule.some((s)=>s.day_of_week === todayDow);
        }).map((c)=>{
            const schedule = c.schedule ?? [];
        ...
        __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$lib$2f$logger$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].error('❌ [DANCE-STUDIO/DASHBOARD/STATS] Erro:', error);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_$40$babel$2b$core$40$7$2e$29$2e$0_$40$opentelemetry$2b$api$40$1$2e$9$2e$0_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: error.message
        }, {
            status: 500
...
//# debugId=7f77effb-c9ee-f563-d4be-3cc6b2e011f2
//# sourceMappingURL=%5Broot-of-the-server%5D__d528a637._.js.map