...
function POSContent() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "money" | "card" | "pix" | "mercadopago_pix" | "credit" | null
  >(null);

  const [cart, setCart] = useState<
    { product: any; quantity: number; type: "product" | "service_order" }[]
  >([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const [inventoryRes, studentsRes] = await Promise.all([
        fetch(`/api/dance-studio/inventory?studioId=${studioId}`).then((r) =>
          r.json(),
        ),
        fetch(`/api/dance-studio/students?studioId=${studioId}`).then(
          async (r) => {
  ...
  const addToCart = (product: Product) => {
    setCart((prev) => {
      const existing = prev.find(
        (item) => item.product.id === product.id && item.type === "product",
      );
      if (existing) {
        return prev.map((item) =>
          item.product.id === product.id && item.type === "product"
  ...
  const addOSToCart = (os: any) => {
    setCart((prev) => {
      const existing = prev.find(
        (item) => item.product.id === os.id && item.type === "service_order",
      );
  ...
  const handleFinalizeSale = async (
    forcedMethod?: "money" | "card" | "pix" | "mercadopago_pix" | "credit",
  ) => {
    console.log("handleFinalizeSale chamada!");
    const method =
      forcedMethod ||
      paymentMethod ||
      (businessModel === "MONETARY" ? "money" : "credit");
    ...
    try {
      const priceInReais = (p: any) =>
        p.selling_price ?? p.price_in_credits ?? 0;
      const items = cart.map((item) => {
        const unitPrice = priceInReais(item.product);
        const creditsPerUnit =
          method === "credit"
            ? unitPrice / Math.max(0.01, reaisPerCredit)
            : ((item.product as any).price_in_credits ?? 0);
        return {
          id: item.product.id,
      ...
      if (method === "card") {
      ...
      } else if (method === "mercadopago_pix") {
        const totalAmount = cart.reduce(
          (acc, i) => acc + (i.product.selling_price ?? 0) * i.quantity,
          0,
        );
        const customerEmail =
        ...
        if (mercadopagoResponse.point_of_interaction?.transaction_data) {
          const qrCodeInfo =
            mercadopagoResponse.point_of_interaction.transaction_data;

          if (qrCodeInfo.qr_code_base64) {
            setPixQRCodeUrl(
              `data:image/png;base64,${qrCodeInfo.qr_code_base64}`,
            );
          } else if (qrCodeInfo.ticket_url) {
            setPixQRCodeUrl(qrCodeInfo.ticket_url); // Fallback if no base64
          }

          if (qrCodeInfo.qr_code) {
            setPixText(qrCodeInfo.qr_code);
          }
          toast({
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <BarcodeScanner
          ...
          onScanSuccess={async (sku) => {
            const r = await fetch(
              `/api/dance-studio/inventory?studioId=${studioId}&sku=${encodeURIComponent(sku)}`,
            );
        ...
        <div className="flex-1 p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  .filter(
                    (p) =>
                      ...
                      (pdvSearchInput.trim() === "" ||
                        p.name
                          ?.toLowerCase()
                          .includes(pdvSearchInput.toLowerCase()) ||
                        p.sku
                          ?.toLowerCase()
                          .includes(pdvSearchInput.toLowerCase())),
                  )
                  ...
                  {pendingOS
                    .filter(
                      (os) =>
                        os.tracking_code
                          ?.toLowerCase()
                          .includes(osSearchInput.toLowerCase()) ||
                        os.customer?.name
                          ?.toLowerCase()
                          .includes(osSearchInput.toLowerCase()),
                    )
                        ...
                        <CardContent className="p-4 flex justify-between items-center">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground truncate">
                              {os.customer?.name || "Cliente Avulso"}
                            </p>
                          </div>
                          <div className="text-right ml-4">
                            <p className="text-indigo-600 font-bold">
                              {formatPrice(os.total_amount)}
                            </p>
                            <Badge
                              variant="outline"
          ...
          <Card className="lg:col-span-4 flex flex-col">
            ...
            <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="Vincular Aluno (Opcional)" />
                </SelectTrigger>
                <SelectContent>
                ...
                >
                  <div className="flex-1 truncate mr-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatPrice(item.product.selling_price)} x{" "}
                      {item.quantity}
                    </p>
                  </div>
                  <Button
            ...
            <div className="p-4 border-t space-y-4">
              <div className="flex justify-between text-xl font-black">
                ...
                <span>
                  {formatPrice(
                    cart.reduce(
                      (acc, i) => acc + i.product.selling_price * i.quantity,
                      0,
                    ),
                  )}
                </span>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-black">
                  {formatPrice(
                    cart.reduce(
                      (acc, i) =>
                        acc + (i.product.selling_price ?? 0) * i.quantity,
                      0,
                    ),
                  )}
                </span>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {cart
                    .reduce(
                      (acc, i) =>
                        acc +
                        ((i.product.selling_price ?? 0) * i.quantity) /
                          reaisPerCredit,
                      0,
                    )
                    .toFixed(2)}{" "}
                  créditos (taxa R$ {reaisPerCredit.toFixed(0)}/crédito)
                </p>
              )}
            </div>
            ...
            <div className="grid gap-4">
              ...
              <div className="flex flex-col gap-2">
                <Button
                  type="button"
                  variant={
                    paymentMethod === "mercadopago_pix" ? "default" : "outline"
                  }
                  className="h-16 justify-start gap-4"
                  onClick={() => setPaymentMethod("mercadopago_pix")}
                >
                  <QrCode className="w-8 h-8" /> PIX (Mercado Pago)
                </Button>
                    ...
                    {!pixQRCodeUrl ? (
                      <>
                        <div className="space-y-2">
                          <Label htmlFor="tax_id">CPF ou CNPJ (Opcional)</Label>
                          <Input
                            id="tax_id"
                        ...
                        <Button
                          ...
                          onClick={() => handleFinalizeSale("mercadopago_pix")}
                      ...
                      <div className="flex flex-col items-center justify-center space-y-4 py-4">
                        <p className="font-medium text-center">
                          Escaneie o QR Code abaixo no seu aplicativo bancário
                        </p>
