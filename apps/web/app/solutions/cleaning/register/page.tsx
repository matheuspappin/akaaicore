...
function RegisterContent() {
    ...
    try {
      ...
      if (response.ok && data.success) {
        toast({
          ...
          description: "Bem-vindo ao ServiçosdeLimpeza! Seu teste grátis começou.",
        })
        ...
        if (role === 'student') router.push("/solutions/cleaning/client")
        else if (role === 'teacher') router.push("/solutions/cleaning/technician")
        else if (role === 'engineer') router.push("/solutions/cleaning/engineer")
        else if (role === 'finance') router.push("/solutions/cleaning/dashboard")
        else router.push("/solutions/cleaning/dashboard")
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      ...
      <Link href="/solutions/cleaning" className="absolute top-6 left-6 z-50 flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
        <ArrowLeft className="w-4 h-4" /> Voltar
      </Link>

      {/* Left Side - Branding */}
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-sky-700 via-sky-600 to-orange-600 p-12 flex-col justify-between relative overflow-hidden">
        ...
        >
          <Link href="/solutions/cleaning" className="flex items-center gap-3 group">
            <motion.div 
            ...
            >
              <FireExtinguisher className="w-6 h-6 text-sky-600" />
            </motion.div>
        ...
        <div className="relative z-10">
          ...
          <ul className="space-y-6">
            {benefits.map((benefit, index) => (
              ...
              >
                <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-md border border-white/30 shadow-lg group-hover:bg-white group-hover:text-sky-600 transition-all duration-300">
        ...
        >
           {/* Box Glow */}
           <div className="absolute inset-0 bg-gradient-to-r from-sky-500/0 via-white/5 to-sky-500/0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />
              ...
              <div className="flex -space-x-3">
                 {[1,2,3].map(i => (
                   <motion.div 
                     ...
                     className="w-10 h-10 rounded-full bg-slate-800 border-2 border-sky-500 flex items-center justify-center text-xs font-black shadow-xl"
                   >
                     U{i}
                   </motion.div>
                 ))}
           ...
           <div className="relative">
             ...
             <p className="text-sm text-white/80 leading-relaxed font-medium pl-2">
               O <span className="text-white font-bold">ServiçosdeLimpeza</span> reduziu nosso tempo de vistoria em 40% e eliminou completamente os erros humanos na gestão de validade de carga.
             </p>
           </div>
          ...
          <Card className="text-card-foreground flex flex-col gap-6 rounded-2xl border border-white/10 py-6 px-6 md:px-8 bg-slate-900 shadow-2xl shadow-black/50">
            ...
            <CardContent className="px-0 pt-0">
              ...
              {(initialStudioId && role === 'finance') ? (
                <div className="flex items-center gap-3 p-4 rounded-xl bg-sky-600/10 border border-sky-500/20 mb-6">
                  ...
                  {[
                    ...
                    <motion.button
                      ...
                      className={cn(
                        ...
                        role === r.id 
                          ? "border-sky-600 bg-sky-600/15 shadow-lg shadow-sky-600/20 text-white" 
                          : "border-white/5 bg-slate-800/50 text-slate-500 hover:border-white/15 hover:text-slate-300 hover:bg-slate-800"
                      )}
                    >
                      <r.icon className={cn("w-5 h-5", role === r.id ? "text-sky-500" : "text-slate-500")} />
                      <span className="text-xs font-bold uppercase tracking-wider">{r.label}</span>
                      <span className={cn("text-[10px] opacity-70 hidden sm:block", role === r.id ? "text-sky-400/80" : "text-slate-500")}>{r.desc}</span>
                    </motion.button>
                  ))}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2 col-span-full">
                    ...
                    <Input
                      ...
                      required
                      className="bg-slate-800/50 border-white/5 text-white h-12 rounded-xl focus:border-sky-500/50"
                    />
                  </div>
                    ...
                    <Label className="flex justify-between text-slate-300 font-bold uppercase tracking-widest text-[10px]">
                      ...
                      {role === 'admin' && (
                        <div className="flex gap-2">
                          <button type="button" onClick={() => setTaxIdType('cpf')} className={cn("px-2 py-0.5 rounded text-[9px] font-black", taxIdType === 'cpf' ? "bg-sky-600 text-white" : "bg-slate-800 text-slate-500")}>CPF</button>
                          <button type="button" onClick={() => setTaxIdType('cnpj')} className={cn("px-2 py-0.5 rounded text-[9px] font-black", taxIdType === 'cnpj' ? "bg-sky-600 text-white" : "bg-slate-800 text-slate-500")}>CNPJ</button>
                        </div>
                      )}
                    </Label>
                    <Input
                  ...
                  <div className="space-y-2 col-span-full sm:col-span-1">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        required
                        className="bg-slate-800/50 border-white/5 text-white h-12 rounded-xl focus:border-sky-500/50 flex-1"
                      ...
                      {!codeSent ? (
                        <Button
                          ...
                          className="h-12 shrink-0 border-white/10 text-white hover:bg-sky-600/20"
                        >
                  ...
                  {role === 'admin' && (
                    <div className="space-y-2 col-span-full">
                      ...
                      <Input
                        placeholder="Ex: ServiçosdeLimpeza Soluções"
                        ...
                        <Card className="border-dashed border-2 border-sky-500/30 bg-sky-500/5">
                          <CardContent className="p-6 space-y-4">
                            ...
                            <div className="pt-4 border-t border-white/10 flex justify-between items-center">
                               ...
                               <span className="text-2xl font-black text-sky-500">R$ {customTotal}</span>
                            </div>
                          </CardContent>
                        </Card>
                      )}
                ...
                <Button
                  type="submit"
                  className="w-full bg-sky-600 hover:bg-sky-700 text-white font-black h-16 rounded-2xl text-lg shadow-xl shadow-sky-600/20 hover:shadow-sky-600/40 hover:scale-[1.02] transition-all"
              ...
              <div className="mt-8 text-center">
                <p className="text-sm text-slate-400 font-medium">
                  Já tem uma conta?{" "}
                  <Link href="/solutions/cleaning/login" className="text-sky-500 hover:text-sky-400 font-bold underline underline-offset-4">
                    Acesse o Login
                  </Link>
                </p>
              </div>
...
export default function RegisterPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-950 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-sky-600" /></div>}>
      <RegisterContent />
    </Suspense>
  )
}
