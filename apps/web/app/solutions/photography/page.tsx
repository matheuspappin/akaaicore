...
function Navbar() {
  ...
  const navItems = [
    ...
    { name: "App do Profissional", href: "#teacher-app" },
      ...
      <div className="container mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 font-bold text-2xl tracking-tight text-white hover:opacity-80 transition-opacity">
          <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-r from-zinc-600 to-pink-600 rounded-lg blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
            <div className="relative w-9 h-9 rounded-lg bg-zinc-600 flex items-center justify-center text-white shadow-lg">
              <Camera className="w-5 h-5" />
            </div>
          </div>
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-white/70">
            AKAAI <span className="text-zinc-400 font-black">Focus</span>
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="px-4 py-2 text-sm font-bold text-white/70 hover:text-zinc-400 transition-all relative group uppercase tracking-widest"
            >
              {item.name}
              <span className="absolute bottom-0 left-4 right-4 h-0.5 bg-zinc-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center" />
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-3">
            <Button size="sm" variant="ghost" className="font-semibold text-white hover:bg-white/10 hover:text-white transition-all" asChild>
              <Link href="/solutions/photography/login">Entrar</Link>
            </Button>
            <Button size="sm" className="hidden sm:flex rounded-full px-6 bg-zinc-600 hover:bg-zinc-700 shadow-lg shadow-zinc-600/20 hover:shadow-zinc-600/40 hover:scale-105 transition-all duration-300 font-bold text-white border-none" asChild>
              <Link href="/solutions/photography/register">Criar Conta Grátis</Link>
            </Button>
            ...
            <div className="flex flex-col p-6 space-y-4 font-semibold text-white">
              {navItems.map((item) => (
                <Link key={item.name} href={item.href} className="flex items-center justify-between group" onClick={() => setMobileMenuOpen(false)}>
                  {item.name} <ChevronRight className="w-4 h-4 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all text-zinc-500" />
                </Link>
              ))}
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                ...
                <Button type="button" className="w-full rounded-xl bg-zinc-600 hover:bg-zinc-700 text-white border-none" asChild>
                  <Link href="/solutions/photography/register">Criar Conta</Link>
                </Button>
              </div>
            </div>
          </motion.div>
    ...
    <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden bg-slate-950/90 text-white">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            Matrículas, turmas, frequência de clientes, pagamentos e comunicação com responsáveis — tudo em um só lugar. Foque na dança, não na burocracia.
          </motion.p>
          ...
          >
            <Button size="lg" className="h-16 px-10 text-xl rounded-full bg-gradient-to-r from-cyan-500 to-fuchsia-600 hover:from-cyan-400 hover:to-fuchsia-500 shadow-[0_0_40px_rgba(0,255,255,0.25)] hover:shadow-[0_0_60px_rgba(168,85,247,0.3)] transition-all hover:scale-105 font-bold text-white border-none group" asChild>
              <Link href="/solutions/photography/register">
                Começar Grátis
          ...
          >
            {[
              { value: "+500", label: "Escolas ativos" },
              { value: "+12k", label: "Clientes gerenciados" },
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Tudo na palma da mão do <span className="text-zinc-400">Profissional</span>
          </h2>
        ...
        <div className="grid md:grid-cols-12 gap-8 items-center max-w-6xl mx-auto">
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                    <div className="bg-white p-4 rounded-2xl shadow-sm mb-4 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-slate-800 font-bold">
                        <ClipboardList className="w-5 h-5 text-zinc-600" /> Chamada
                      </div>
                      <Badge className="bg-violet-100 text-zinc-600 border-none">Turma Ballet</Badge>
                    </div>
                    <div className="space-y-2 flex-1 overflow-auto">
                      {[
                        ...
                        <div key={cliente.name} className={cn(
                          "bg-white p-3 rounded-xl shadow-sm flex items-center justify-between border-l-4",
                          cliente.present ? "border-l-emerald-500" : "border-l-red-400"
                        )}>
                          <div className="flex items-center gap-2">
                            <div className={cn(
                              "w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white",
                              cliente.present ? "bg-emerald-500" : "bg-slate-300"
                            )}>
                              {cliente.name[0]}
                            </div>
                            <span className="text-sm font-semibold text-slate-700">{cliente.name}</span>
                          </div>
                          {cliente.present
                    ...
                    <Button size="sm" className="mt-4 bg-zinc-600 text-white w-full rounded-xl">
                      Salvar Chamada
                    </Button>
                  </motion.div>
                )}
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { name: "Jazz Adulto", time: "11:00", students: 8, color: "border-l-zinc-400" },
                        ...
                        <div key={turma.name} className={cn("bg-white p-4 rounded-2xl shadow-sm border-l-4", turma.color)}>
                          ...
                          <div className="flex items-center justify-between mt-1">
                            ...
                            <div className="flex items-center gap-1 text-xs text-slate-500">
                              <Users className="w-3 h-3" /> {turma.students} clientes
                            </div>
                          </div>
          ...
          <div className="md:col-span-7 space-y-6">
            {[
              { id: 'chamada', title: 'Chamada Digital', desc: 'Frequência em segundos, sem papel. Histórico completo por cliente.', icon: ClipboardList },
              { id: 'turma', title: 'Agenda de Turmas', desc: 'Visualize todas as turmas do dia com horários e número de clientes.', icon: Calendar },
              { id: 'qrcode', title: 'Check-in por QR Code', desc: 'Clientes fazem check-in autônomo na entrada do escola.', icon: Smartphone }
            ].map((feature) => (
              <div
                key={feature.id}
                ...
                className={cn(
                  ...
                  activeTab === feature.id ? "bg-slate-800 border-zinc-500/30 shadow-lg" : "bg-slate-900/50 border-white/5"
                )}
              >
                <div className="flex gap-4">
                  <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", activeTab === feature.id ? "bg-zinc-600 text-white" : "bg-slate-700 text-slate-400")}>
    ...
    <section id="guardian-portal" className="py-24 bg-slate-950/90 text-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-12 gap-8 items-center max-w-6xl mx-auto">
          ...
          <div className="md:col-span-5 flex justify-center">
            <div className="relative w-[300px] h-[600px] bg-slate-950 rounded-[3rem] border-8 border-slate-800 shadow-2xl overflow-hidden">
              ...
              <div className="w-full h-full bg-slate-50 pt-10 pb-4 px-4 flex flex-col">
                {/* Header portal */}
                <div className="bg-zinc-700 p-4 rounded-2xl shadow-xl border border-zinc-600 mb-4">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-sm text-white">Fotografia(Escola) — Portal</span>
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div className="bg-white p-3 rounded-xl text-center shadow-sm">
                    <p className="text-2xl font-black text-zinc-600">92%</p>
                  ...
                  <div className="bg-white p-3 rounded-xl text-center shadow-sm">
                    ...
                    <p className="text-[10px] font-bold uppercase text-slate-400">Cursos em dia</p>
                  </div>
                </div>
                {/* Comunicados */}
                <div className="bg-white p-3 rounded-xl shadow-sm flex-1 overflow-auto">
                  ...
                  {[
                    { text: "Recital de Inverno: 15/06 às 19h", icon: Star, color: "text-zinc-500" },
                    { text: "Reposição de curso: Sábado 10h", icon: Calendar, color: "text-pink-500" },
                  ].map((item, i) => (
          ...
          <div className="md:col-span-7 space-y-6">
            {[
              { title: 'Frequência em Tempo Real', desc: 'Pais acompanham cada curso e recebem alertas de falta.', icon: TrendingUp, color: "bg-zinc-600" },
...
function ManagementDashboard() {
  const stats = [
    { label: "Clientes Ativos", value: "124", icon: Users, color: "text-zinc-500", trend: "+8 este mês" },
  ...
  const modules = [
    { label: "Turmas", icon: GraduationCap, color: "bg-zinc-600", shadow: "shadow-zinc-500/30", description: "Crie e gerencie turmas por modalidade e nível." },
    { label: "Clientes", icon: Users, color: "bg-pink-600", shadow: "shadow-pink-500/30", description: "Cadastro completo com histórico de frequência." },
    ...
    { label: "Profissionais", icon: Heart, color: "bg-rose-600", shadow: "shadow-rose-500/30", description: "Agenda, pagamentos e avaliações de profissionais." },
    ...
    { label: "Matrículas", icon: UserPlus, color: "bg-teal-600", shadow: "shadow-teal-500/30", description: "Fluxo de captação e onboarding de novos clientes." },
    ...
    { label: "Gamificação", icon: Trophy, color: "bg-yellow-500", shadow: "shadow-yellow-500/30", description: "Rankings, conquistas e engajamento dos clientes." },
  ]

  return (
    <section id="management" className="py-24 bg-slate-950/90 relative overflow-hidden">
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
        <div className="absolute top-[20%] right-[-5%] w-[400px] h-[400px] bg-zinc-600/5 rounded-full blur-[100px]" />
        <div className="absolute bottom-[10%] left-[-5%] w-[400px] h-[400px] bg-pink-600/5 rounded-full blur-[100px]" />
      </div>
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <Badge className="bg-zinc-500/20 text-zinc-400 border-zinc-500/30 px-4 py-1 mb-4 uppercase tracking-widest text-[10px] font-black">Painel Administrativo</Badge>
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Controle total do seu <span className="text-zinc-400">Escola</span>
          </h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">
                  ...
                  <Tooltip key={i}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <span className="text-[10px] sm:text-xs font-bold text-slate-600 text-center uppercase tracking-tighter group-hover:text-zinc-500 transition-colors">
                          {mod.label}
                        </span>
                      </motion.div>
                    </TooltipTrigger>
                    <TooltipContent side="bottom" className="max-w-[200px] text-center bg-slate-900 text-white border-slate-800 p-3 rounded-xl shadow-xl">
                      <p className="font-bold mb-1 text-zinc-400 uppercase tracking-tighter text-[10px]">{mod.label}</p>
...
function FeatureGrid() {
  const features = [
    { title: "Controle de Mensalidades", description: "Cobranças automáticas com boleto, cartão ou Pix. Inadimplência em destaque.", icon: CreditCard, color: "from-zinc-500 to-purple-400" },
    ...
    { title: "Captação de Clientes", description: "Formulário de matrícula online e funil de leads integrado.", icon: UserPlus, color: "from-pink-500 to-rose-400" },
  ...
  return (
    <section id="features" className="py-32 bg-slate-50 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-20">
        <div className="absolute top-[10%] left-[5%] w-96 h-96 bg-zinc-400 rounded-full blur-[128px]" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Badge className="bg-violet-100 text-zinc-600 border-none px-4 py-1.5 mb-6 uppercase tracking-[0.2em] text-[10px] font-black shadow-sm">
              Recursos
            </Badge>
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">
              Tudo em um <span className="text-transparent bg-clip-text bg-gradient-to-r from-zinc-600 to-pink-600">só lugar</span>
            </h2>
            <p className="text-slate-500 max-w-2xl mx-auto text-lg leading-relaxed">
              A plataforma mais completa para modernizar a gestão do seu fotografia (escola) e encantar clientes e responsáveis.
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {features.map((feature, i) => (
            <motion.div
              ...
              transition={{ delay: i * 0.1 }}
              whileHover={{ y: -10 }}
              className="group bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm hover:shadow-2xl hover:shadow-zinc-500/10 transition-all duration-500 relative overflow-hidden"
            >
              <div className="absolute -inset-2 bg-gradient-to-r from-zinc-500/0 to-zinc-500/0 group-hover:from-zinc-500/5 group-hover:to-zinc-500/5 transition-all duration-500 blur-xl opacity-0 group-hover:opacity-100" />
              <div className={cn(
              ...
              <h3 className="text-xl font-black text-slate-900 mb-3 group-hover:text-zinc-600 transition-colors">{feature.title}</h3>
...
function PricingSection() {
  const highlights = [
    ...
    { icon: "📱", label: "Treinamento completo", desc: "Capacitação da equipe de profissionais e recepção" },
    { icon: "💬", label: "Suporte dedicado", desc: "Atendimento direto com especialistas em gestão de dança" },
    { icon: "🎯", label: "Personalização total", desc: "Sistema configurado para o fluxo do seu escola" },
  ]

  return (
    <section id="pricing" className="py-32 bg-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10 text-center max-w-4xl">
        <Badge className="bg-violet-100 text-zinc-600 border-none px-4 py-1.5 mb-6 uppercase tracking-[0.2em] text-[10px] font-black">
          Implementação
        </Badge>
        <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">
          Instalado pelo <span className="text-transparent bg-clip-text bg-gradient-to-r from-zinc-600 to-pink-600">nosso time</span>
        </h2>
        <p className="text-slate-500 max-w-2xl mx-auto text-lg mb-16">
          O Fotografia(Escola) é implantado com suporte especializado. Entre em contato para que nossa equipe entenda o seu escola e configure a plataforma perfeita para vocês.
        </p>
        ...
        <div className="bg-gradient-to-br from-zinc-600 to-pink-600 rounded-3xl p-10 text-white shadow-2xl shadow-zinc-500/20">
          <h3 className="text-2xl font-black mb-3">Pronto para transformar seu escola?</h3>
          <p className="text-violet-100 mb-8">
            Fale com nosso time e receba uma demonstração exclusiva do Fotografia(Escola) para o seu escola.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href="https://wa.me/5511999999999?text=Olá! Tenho interesse no Fotografia(Escola) para meu fotografia (escola)."
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 bg-white text-zinc-700 hover:bg-violet-50 font-bold h-14 px-8 rounded-2xl transition-all shadow-lg"
            >
              <Camera className="w-5 h-5" />
...
function CTASection() {
  return (
    <section className="py-32 bg-slate-950/90 relative overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-from)_0%,_transparent_70%)] from-zinc-900/30 via-transparent to-transparent" />
      </div>
      <div className="container mx-auto px-6 relative z-10 text-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-zinc-500/30 bg-zinc-500/10 text-zinc-400 font-bold text-sm mb-8">
            <Sparkles className="w-4 h-4" />
            14 dias grátis, sem cartão
          </div>
          <h2 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">
            Pronto para transformar <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-zinc-400 to-pink-400">seu escola?</span>
          </h2>
          <p className="text-slate-400 text-xl max-w-2xl mx-auto mb-12">
            Junte-se a centenas de escolas que já modernizaram sua gestão com o Fotografia(Escola).
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button size="lg" className="h-16 px-12 text-xl rounded-full bg-zinc-600 hover:bg-zinc-700 shadow-[0_10px_40px_-10px_rgba(139,92,246,0.6)] hover:scale-105 transition-all font-bold text-white border-none group" asChild>
              <Link href="/solutions/photography/register">
                Criar Conta Grátis
                <ArrowRight className="ml-2 w-6 h-6 group-hover:translate-x-1 transition-transform" />
              </Link>
            ...
            <Button size="lg" variant="outline" className="h-16 px-12 text-xl rounded-full border-2 border-white/10 hover:bg-white/10 hover:text-white font-bold text-slate-300 bg-transparent" asChild>
              <Link href="/solutions/photography/login">Já tenho conta</Link>
            </Button>
          </div>
        </motion.div>
      </div>
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-12 pb-12 border-b border-white/5">
          <div className="md:col-span-5 space-y-6">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-lg bg-zinc-600 flex items-center justify-center shadow-lg shadow-zinc-600/20">
            ...
            <p className="text-slate-400 text-sm leading-relaxed max-w-sm">
              A plataforma definitiva para gestão de fotografia (escola). 
              Foque na arte e na evolução dos seus clientes, nós cuidamos da burocracia.
            </p>
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              O ecossistema **akaaicore** fornece a infraestrutura robusta, modular e baseada em IA que alimenta o Fotografia(Escola), 
          ...
          <div className="md:col-span-3 space-y-6">
            ...
            <nav className="flex flex-col gap-3">
              <Link href="#" className="text-slate-500 hover:text-slate-300 text-sm transition-colors flex items-center gap-2 group">
                <div className="w-1 h-1 rounded-full bg-zinc-500/50 group-hover:bg-zinc-500 transition-colors" />
                Privacidade
              </Link>
              <Link href="#" className="text-slate-500 hover:text-slate-300 text-sm transition-colors flex items-center gap-2 group">
                <div className="w-1 h-1 rounded-full bg-zinc-500/50 group-hover:bg-zinc-500 transition-colors" />
              ...
              <Link href="/solutions/photography/login" className="text-slate-500 hover:text-zinc-400 text-sm transition-colors flex items-center gap-2 group">
                <div className="w-1 h-1 rounded-full bg-zinc-500/50 group-hover:bg-zinc-400 transition-colors" />
                Login no Sistema
              </Link>
            </nav>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-4">
          <p className="text-slate-600 text-[10px] uppercase tracking-[0.2em] font-black">
            © {new Date().getFullYear()} Fotografia(Escola) — powered by akaaicore. Todos os direitos reservados.
          </p>
