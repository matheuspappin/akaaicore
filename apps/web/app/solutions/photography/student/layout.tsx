...
const navItems = [
  { href: "/solutions/photography/student",            label: "Início",        icon: LayoutDashboard },
  { href: "/solutions/photography/student/turmas",     label: "Minhas Turmas", icon: Calendar },
  { href: "/solutions/photography/student/financeiro", label: "Créditos",      icon: DollarSign },
  { href: "/solutions/photography/student/perfil",     label: "Meu Perfil",    icon: UserCircle },
]
...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/photography/login")
        return
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-16 flex items-center justify-between gap-4 px-4 md:px-6 bg-slate-950/95 border-b border-white/10 backdrop-blur-md">
        <Link href="/solutions/photography/student" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-zinc-600 flex items-center justify-center shadow-lg shadow-zinc-600/25">
            <Music className="w-5 h-5 text-white" />
          </div>
          <div>
            <span className="font-black text-white tracking-tighter text-base block leading-tight">Fotografia(Escola)</span>
            <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">Portal do Aluno</span>
          </div>
        </Link>

        <div className="flex items-center gap-3">
          <Link
            href="/solutions/photography/student/financeiro"
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-zinc-600/30 hover:bg-zinc-600/50 text-zinc-300 hover:text-white transition-all border border-zinc-500/20"
          >
          ...
          <button
            ...
            disabled={loggingOut}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-white/60 hover:text-zinc-400 hover:bg-zinc-600/10 text-xs font-bold uppercase tracking-widest transition-all disabled:opacity-50"
          >
      ...
      <nav className="hidden md:flex bg-white dark:bg-slate-900/50 border-b border-slate-200 dark:border-white/10 px-4 md:px-6">
        <div className="flex gap-1 max-w-4xl w-full">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== "/solutions/photography/student" && pathname.startsWith(item.href))
            return (
              <Link
                key={item.href}
                href={item.href}
                className={cn(
                  ...
                  isActive
                    ...
                    : "text-slate-500 hover:text-zinc-600 border-transparent hover:border-zinc-600/50"
                )}
              >
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "text-zinc-600 bg-violet-50/50 dark:bg-zinc-600/10"
