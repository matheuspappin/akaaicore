...
function RegisterContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        toast({ title: "Conta criada com sucesso!", description: inviteStudioName ? `Bem-vindo ao ${inviteStudioName}!` : "Bem-vindo ao Fotografia(Escola)!" })
        await new Promise(resolve => setTimeout(resolve, 400))
        if (role === 'student') router.push("/solutions/photography/student")
        else if (role === 'teacher') router.push("/solutions/photography/teacher")
        else router.push("/solutions/photography/dashboard")
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      <Link href="/solutions/photography" className="absolute top-6 left-6 z-50 flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
        <ArrowLeft className="w-4 h-4" /> Voltar
      </Link>

      {/* Left Side */}
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-zinc-800 via-zinc-700 to-pink-700 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <Link href="/solutions/photography" className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center shadow-2xl">
            <Music className="w-6 h-6 text-zinc-600" />
          </div>
          <span className="text-2xl font-black text-white tracking-tighter">
            AKAAI <span className="text-violet-200">Focus</span>
          </span>
        </Link>
        ...
        <div className="bg-black/20 backdrop-blur-xl p-8 rounded-[2rem] border border-white/10">
          <div className="flex items-center gap-4 mb-4">
            <div className="flex -space-x-3">
              {[1, 2, 3].map(i => (
                <div key={i} className="w-10 h-10 rounded-full bg-slate-800 border-2 border-zinc-500 flex items-center justify-center text-xs font-black text-white">
          ...
          <p className="text-sm text-white/80 leading-relaxed font-medium">
            O <span className="text-white font-bold">Fotografia(Escola)</span> transformou minha gestão. Antes eu passava horas com planilhas, agora tudo é automático.
          </p>
            ...
            <CardContent className="px-0 pt-4">
              {inviteCode ? (
                <div className="flex items-center gap-3 p-4 rounded-xl bg-zinc-600/10 border border-zinc-500/20 mb-6">
                  <Music className="w-5 h-5 text-zinc-500 shrink-0" />
                  <div>
                    {inviteLoading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                  {[
                    ...
                    >
                      ...
                      <span className={cn("text-[10px] opacity-70", role === r.id ? "text-zinc-400/80" : "text-slate-500")}>{r.desc}</span>
                    </button>
                  ))}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <Button
                  type="submit"
                  className="w-full bg-zinc-600 hover:bg-zinc-700 text-white font-black h-16 rounded-2xl text-lg shadow-xl shadow-zinc-600/20 hover:scale-[1.02] transition-all"
              ...
              <div className="mt-8 text-center">
                <p className="text-sm text-slate-400 font-medium">
                  Já tem uma conta?{" "}
                  <Link href="/solutions/photography/login" className="text-zinc-500 hover:text-zinc-400 font-bold underline underline-offset-4">
                    Fazer Login
                  </Link>
                </p>
              </div>
