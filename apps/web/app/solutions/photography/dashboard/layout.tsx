...
function DanceSidebar({
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/photography")
  }
        ...
        {!collapsed && (
          <Link href="/solutions/photography" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              AKAAI <span className="text-zinc-400">Focus</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function DanceDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (role === 'student') {
        router.push("/solutions/photography/student")
        return
      }
      if (role === 'teacher') {
        router.push("/solutions/photography/teacher")
        return
      }
      setAuthorized(true)
      setLoading(false)
