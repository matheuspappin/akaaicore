...
const navItems = [
  { href: "/solutions/yoga/student",            label: "Início",        icon: LayoutDashboard },
  { href: "/solutions/yoga/student/turmas",     label: "Minhas Turmas", icon: Calendar },
  { href: "/solutions/yoga/student/financeiro", label: "Créditos",      icon: DollarSign },
  { href: "/solutions/yoga/student/perfil",     label: "Meu Perfil",    icon: UserCircle },
]
...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/yoga/login")
        return
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-16 flex items-center justify-between gap-4 px-4 md:px-6 bg-slate-950/95 border-b border-white/10 backdrop-blur-md">
        <Link href="/solutions/yoga/student" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-teal-600 flex items-center justify-center shadow-lg shadow-teal-600/25">
            <Music className="w-5 h-5 text-white" />
          </div>
          <div>
            <span className="font-black text-white tracking-tighter text-base block leading-tight">Yoga</span>
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/solutions/yoga/student/financeiro"
      ...
      <nav className="hidden md:flex bg-white dark:bg-slate-900/50 border-b border-slate-200 dark:border-white/10 px-4 md:px-6">
        <div className="flex gap-1 max-w-4xl w-full">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== "/solutions/yoga/student" && pathname.startsWith(item.href))
            return (
              <Link
                key={item.href}
                href={item.href}
