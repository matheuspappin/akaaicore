      ...
      <div className="flex gap-2 flex-wrap">
        {SUGGESTIONS.map(s => (
          <button
            key={s}
            onClick={() => send(s)}
            className="px-3 py-1.5 rounded-full text-xs font-bold bg-violet-100 text-teal-700 dark:bg-teal-600/20 dark:text-teal-400 hover:bg-violet-200 dark:hover:bg-teal-600/30 transition-colors"
          >
            {s}
          </button>
        ))}
          ...
          {messages.map((msg) => (
            ...
            >
              <div className={cn(
                ...
                msg.role === "assistant" ? "bg-teal-600 text-white" : "bg-slate-200 dark:bg-slate-700"
              )}>
