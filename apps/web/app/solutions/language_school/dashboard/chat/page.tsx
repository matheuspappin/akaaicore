      ...
      <div className="flex gap-2 flex-wrap">
        {SUGGESTIONS.map(s => (
          <button
            key={s}
            onClick={() => send(s)}
            className="px-3 py-1.5 rounded-full text-xs font-bold bg-violet-100 text-indigo-700 dark:bg-indigo-600/20 dark:text-indigo-400 hover:bg-violet-200 dark:hover:bg-indigo-600/30 transition-colors"
          >
            {s}
          </button>
        ))}
          ...
          {messages.map((msg) => (
            ...
            >
              <div className={cn(
                ...
                msg.role === "assistant" ? "bg-indigo-600 text-white" : "bg-slate-200 dark:bg-slate-700"
              )}>
