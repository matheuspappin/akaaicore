...
function DanceSidebar({
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/language_school")
  }
        ...
        {!collapsed && (
          <Link href="/solutions/language_school" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              AKAAI <span className="text-indigo-400">Lingo</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function DanceDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (role === 'student') {
        router.push("/solutions/language_school/student")
        return
      }
      if (role === 'teacher') {
        router.push("/solutions/language_school/teacher")
        return
      }
      setAuthorized(true)
      setLoading(false)
