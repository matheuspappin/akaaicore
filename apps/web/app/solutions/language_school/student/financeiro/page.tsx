...
const usageConfig: Record<UsageEntry['usage_type'], { label: string; icon: any; color: string; sign: string }> = {
  class_attendance: { label: 'Aula realizada',    icon: Zap,         color: 'bg-violet-100 text-indigo-600 dark:bg-indigo-600/20', sign: '-' },
    ...
    try {
      const res = await fetch('/api/dance-studio/packages/checkout', {
        ...
        body: JSON.stringify({
          ...
          successUrl: `${window.location.origin}/solutions/language_school/student/financeiro?payment=success&session_id={CHECKOUT_SESSION_ID}`,
          cancelUrl: `${window.location.origin}/solutions/language_school/student/financeiro?payment=cancelled`,
        }),
      })
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isPopular
                      ? 'border-indigo-500 bg-violet-50 dark:bg-violet-950/20 shadow-md shadow-indigo-500/10'
                      : 'border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/50 hover:border-indigo-300 dark:hover:border-indigo-700/50'
                  )}
                >
                  {isPopular && (
                    <div className="absolute top-0 right-0 bg-indigo-600 text-white text-[10px] font-black px-3 py-1 rounded-bl-xl">
                      MAIS POPULAR
                    </div>
                  )}
                  ...
                  <div className="p-4 flex items-center gap-4">
                    ...
                    <div className={cn(
                      ...
                      isPopular
                        ...
                        : 'bg-violet-100 dark:bg-indigo-600/20 text-indigo-700 dark:text-indigo-400'
                    )}>
            ...
            <CardContent className="p-0">
              ...
              {usage.length > 5 && (
                <button
                  ...
                  className="w-full py-3 text-xs font-bold text-indigo-600 hover:text-indigo-700 hover:bg-violet-50 dark:hover:bg-violet-950/20 transition-colors flex items-center justify-center gap-1.5 border-t border-slate-100 dark:border-white/5"
                >
                  {showAllUsage ? (
