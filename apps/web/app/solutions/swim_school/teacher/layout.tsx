...
const navItems = [
  { href: "/solutions/swim_school/teacher",         label: "Dashboard",   icon: LayoutDashboard },
  { href: "/solutions/swim_school/teacher/turmas",  label: "Minhas Turmas", icon: Calendar },
  { href: "/solutions/swim_school/teacher/chamada", label: "Chamada",     icon: ClipboardList },
  { href: "/solutions/swim_school/teacher/pagamentos", label: "Meus Pagamentos", icon: DollarSign },
  { href: "/solutions/swim_school/teacher/perfil",  label: "Meu Perfil",  icon: UserCircle },
]
  ...
  const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        <Link href="/solutions/swim_school" className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-pink-600 flex items-center justify-center">
            <GraduationCap className="w-4 h-4 text-white" />
          </div>
          {(!collapsed || isMobile) && (
            <div>
              <span className="font-black text-white tracking-tighter text-xs block">Natação</span>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950 border-b border-white/10 flex items-center justify-between px-4 z-40">
        <div className="flex items-center gap-2">
          ...
          <span className="font-black text-white text-sm">
            Natação <span className="text-pink-400">Professor</span>
          </span>
        </div>
