...
const statusMap: Record<string, { label: string; icon: any; className: string }> = {
  ...
  nao_conforme: { label: "Não Conforme",icon: XCircle,     className: "bg-red-100 text-blue-700 dark:bg-blue-600/20 dark:text-blue-400" },
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div>
          ...
          {total > 0 && (
            <div className="mb-4 space-y-1.5">
              <Progress
                value={scoreAuto}
                className={cn("h-2", scoreAuto >= 80 ? "[&>div]:bg-emerald-500" : scoreAuto >= 50 ? "[&>div]:bg-amber-500" : "[&>div]:bg-blue-500")}
              />
              <div className="flex items-center justify-between text-xs text-slate-500">
                <span>Conformidade automática</span>
                <span className={cn(
                  "font-black",
                  scoreAuto >= 80 ? "text-emerald-600" : scoreAuto >= 50 ? "text-amber-600" : "text-blue-600"
                )}>
                  {scoreAuto}%
                </span>
              </div>
        ...
        <div className={cn(
          ...
          isConforme
            ? "bg-emerald-50 border-emerald-200 dark:bg-emerald-600/10 dark:border-emerald-600/30"
            : "bg-red-50 border-red-200 dark:bg-blue-600/10 dark:border-blue-600/30"
        )}>
          {isConforme
          ...
          <div>
            <p className={cn(
              "text-sm font-bold",
              isConforme ? "text-emerald-700 dark:text-emerald-400" : "text-blue-700 dark:text-blue-400"
            )}>
              {isConforme ? `Conforme — Score ${score}%` : `Não Conforme — Score ${score}%`}
            </p>
            <p className={cn(
              "text-xs mt-0.5",
              isConforme ? "text-emerald-600 dark:text-emerald-500" : "text-blue-600 dark:text-blue-500"
            )}>
              {isConforme
  ...
  return (
    <div className="space-y-6">
      ...
      {counts.nao_conforme > 0 && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-red-50 dark:bg-blue-600/10 border border-red-200 dark:border-blue-600/30">
          ...
          <Button size="sm" variant="outline" className="ml-auto border-blue-300 text-blue-700 hover:bg-red-100"
        ...
        <div className="flex gap-2 flex-wrap">
          {[
            ...
            <Button
              ...
              size="sm"
              className={cn(filtro === f.key && "bg-blue-600 hover:bg-blue-700 border-blue-600")}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <span className="text-xs font-black text-blue-600 bg-red-50 dark:bg-blue-600/10 px-2 py-0.5 rounded-lg">
                          {vst.tracking_code}
                        </span>
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-3 text-xs text-slate-500 dark:text-slate-400">
                        ...
                        {vst.conformidade_score !== null && vst.conformidade_score !== undefined && (
                          <span className={cn(
                            ...
                            vst.conformidade_score >= 80 ? "text-emerald-600" : "text-blue-600"
                          )}>
