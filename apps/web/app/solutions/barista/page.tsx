...
import { FireControlLogoCanvas as CafeteriaBaristaLogoCanvas } from "@/components/fire-protection/fire-control-logo-canvas"

import { OFFICIAL_LOGO } from "@/config/branding"

export default function CafeteriaBaristaPrePage() {
  useEffect(() => {
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6">
        ...
        >
          <div className="flex flex-col items-center gap-4 mb-6">
            <div className="flex items-center justify-center gap-3">
              <motion.div
                className="w-14 h-14 flex-shrink-0 rounded-xl bg-amber-600 flex items-center justify-center shadow-[0_0_40px_rgba(220,38,38,0.5)]"
                animate={{
                  boxShadow: [
              ...
              <div className="h-14 md:h-16 min-w-[280px] md:min-w-[360px] w-full max-w-[360px] flex items-center justify-center overflow-visible" aria-label="Cafeteria/Barista">
                <CafeteriaBaristaLogoCanvas className="w-full h-full" text="CafeteriaBarista" />
              </div>
            </div>
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-300 text-xs font-bold uppercase tracking-widest backdrop-blur-sm">
              Cafeteria/Barista
            </span>
          </div>
          <p className="text-slate-300 text-lg md:text-xl max-w-md mx-auto font-medium">
            Gestão inteligente para Cafeteria/Barista
          </p>
        </motion.div>
        ...
        >
          <Button
            ...
            asChild
          >
            <Link href="/solutions/barista/landing">
              <span className="absolute inset-0 bg-gradient-to-r from-orange-500/0 via-amber-500/0 to-orange-500/0 group-hover:from-orange-400/10 group-hover:via-amber-500/10 group-hover:to-orange-400/10 transition-colors duration-300" />
        ...
        >
          <Button
            ...
            asChild
          >
            <Link href="/solutions/barista/login">
              Já tenho conta
            </Link>
          </Button>
        </motion.div>
