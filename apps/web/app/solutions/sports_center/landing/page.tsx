...
import { Trophy,  useState, useEffect, useRef } from "react"
...
import {
  Check, ArrowRight, ArrowLeft, Trophy, CreditCard, Calendar,
  DollarSign, Layout,
      ...
      <div className="container mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 font-bold text-2xl tracking-tight text-white hover:opacity-80 transition-opacity">
          <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-r from-emerald-600 to-pink-600 rounded-lg blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
            <div className="relative w-9 h-9 rounded-lg bg-emerald-600 flex items-center justify-center text-white shadow-lg">
              <Trophy className="w-5 h-5" />
            </div>
          </div>
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-white/70">
            AKAAI <span className="text-emerald-400 font-black">Court</span>
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="px-4 py-2 text-sm font-bold text-white/70 hover:text-emerald-400 transition-all relative group uppercase tracking-widest"
            >
              {item.name}
              <span className="absolute bottom-0 left-4 right-4 h-0.5 bg-emerald-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center" />
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-3">
            ...
            <Button size="sm" className="hidden sm:flex rounded-full px-6 bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 hover:shadow-emerald-600/40 hover:scale-105 transition-all duration-300 font-bold text-white border-none" asChild>
            ...
            <div className="flex flex-col p-6 space-y-4 font-semibold text-white">
              {navItems.map((item) => (
                <Link key={item.name} href={item.href} className="flex items-center justify-between group" onClick={() => setMobileMenuOpen(false)}>
                  {item.name} <ChevronRight className="w-4 h-4 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all text-emerald-500" />
                </Link>
              ))}
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                ...
                <Button type="button" className="w-full rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white border-none" asChild>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Tudo na palma da mão do <span className="text-emerald-400">Instrutor</span>
          </h2>
        ...
        <div className="grid md:grid-cols-12 gap-8 items-center max-w-6xl mx-auto">
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                    <div className="bg-white p-4 rounded-2xl shadow-sm mb-4 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-slate-800 font-bold">
                        <ClipboardList className="w-5 h-5 text-emerald-600" /> Chamada
                      </div>
                      <Badge className="bg-emerald-100 text-emerald-600 border-none">Turma Ballet</Badge>
                    </div>
                    ...
                    <Button size="sm" className="mt-4 bg-emerald-600 text-white w-full rounded-xl">
                      Salvar Chamada
                    </Button>
                  </motion.div>
                )}
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { name: "Jazz Adulto", time: "11:00", students: 8, color: "border-l-emerald-400" },
          ...
          <div className="md:col-span-7 space-y-6">
            {[
              ...
              { id: 'qrcode', title: 'Check-in por QR Code', desc: 'Atletas fazem check-in autônomo na entrada do estúdio.', icon: Smartphone }
            ].map((feature) => (
              <div
                key={feature.id}
                ...
                className={cn(
                  ...
                  activeTab === feature.id ? "bg-slate-800 border-emerald-500/30 shadow-lg" : "bg-slate-900/50 border-white/5"
                )}
              >
                <div className="flex gap-4">
                  <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", activeTab === feature.id ? "bg-emerald-600 text-white" : "bg-slate-700 text-slate-400")}>
    ...
    <section id="guardian-portal" className="py-24 bg-slate-950/90 text-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-12 gap-8 items-center max-w-6xl mx-auto">
          ...
          <div className="md:col-span-5 flex justify-center">
            <div className="relative w-[300px] h-[600px] bg-slate-950 rounded-[3rem] border-8 border-slate-800 shadow-2xl overflow-hidden">
              ...
              <div className="w-full h-full bg-slate-50 pt-10 pb-4 px-4 flex flex-col">
                {/* Header portal */}
                <div className="bg-emerald-700 p-4 rounded-2xl shadow-xl border border-emerald-600 mb-4">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-sm text-white">AKAAI Court — Portal</span>
                ...
                <div className="bg-white p-3 rounded-xl shadow-sm flex-1 overflow-auto">
                  ...
                  {[
                    { text: "Recital de Inverno: 15/06 às 19h", icon: Star, color: "text-emerald-500" },
          ...
          <div className="md:col-span-7 space-y-6">
            {[
              { title: 'Frequência em Tempo Real', desc: 'Pais acompanham cada aula e recebem alertas de falta.', icon: TrendingUp, color: "bg-emerald-600" },
...
function ManagementDashboard() {
  const stats = [
    { label: "Atletas Ativos", value: "124", icon: Users, color: "text-emerald-500", trend: "+8 este mês" },
  ...
  const modules = [
    { label: "Turmas", icon: GraduationCap, color: "bg-emerald-600", shadow: "shadow-emerald-500/30", description: "Crie e gerencie turmas por modalidade e nível." },
    ...
    { label: "Instrutores", icon: Heart, color: "bg-rose-600", shadow: "shadow-rose-500/30", description: "Agenda, pagamentos e avaliações de professores." },
  ...
  return (
    <section id="management" className="py-24 bg-slate-950/90 relative overflow-hidden">
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
        <div className="absolute top-[20%] right-[-5%] w-[400px] h-[400px] bg-emerald-600/5 rounded-full blur-[100px]" />
        <div className="absolute bottom-[10%] left-[-5%] w-[400px] h-[400px] bg-pink-600/5 rounded-full blur-[100px]" />
      </div>
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 px-4 py-1 mb-4 uppercase tracking-widest text-[10px] font-black">Painel Administrativo</Badge>
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Controle total do seu <span className="text-emerald-400">Centro</span>
          </h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">
                  ...
                  <Tooltip key={i}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <span className="text-[10px] sm:text-xs font-bold text-slate-600 text-center uppercase tracking-tighter group-hover:text-emerald-500 transition-colors">
                          {mod.label}
                        </span>
                      </motion.div>
                    </TooltipTrigger>
                    <TooltipContent side="bottom" className="max-w-[200px] text-center bg-slate-900 text-white border-slate-800 p-3 rounded-xl shadow-xl">
                      <p className="font-bold mb-1 text-emerald-400 uppercase tracking-tighter text-[10px]">{mod.label}</p>
...
function FeatureGrid() {
  const features = [
    { title: "Controle de Mensalidades", description: "Cobranças automáticas com boleto, cartão ou Pix. Inadimplência em destaque.", icon: CreditCard, color: "from-emerald-500 to-purple-400" },
    ...
    { title: "Multi-Esporte", description: "Ballet, jazz, funk, sapateado, contemporâneo e muito mais.", icon: Trophy, color: "from-amber-500 to-orange-400" },
  ]

  return (
    <section id="features" className="py-32 bg-slate-50 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-20">
        <div className="absolute top-[10%] left-[5%] w-96 h-96 bg-emerald-400 rounded-full blur-[128px]" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Badge className="bg-emerald-100 text-emerald-600 border-none px-4 py-1.5 mb-6 uppercase tracking-[0.2em] text-[10px] font-black shadow-sm">
              Recursos
            </Badge>
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">
              Tudo em um <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-pink-600">só lugar</span>
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {features.map((feature, i) => (
            <motion.div
              ...
              transition={{ delay: i * 0.1 }}
              whileHover={{ y: -10 }}
              className="group bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500 relative overflow-hidden"
            >
              <div className="absolute -inset-2 bg-gradient-to-r from-emerald-500/0 to-emerald-500/0 group-hover:from-emerald-500/5 group-hover:to-emerald-500/5 transition-all duration-500 blur-xl opacity-0 group-hover:opacity-100" />
              <div className={cn(
              ...
              <h3 className="text-xl font-black text-slate-900 mb-3 group-hover:text-emerald-600 transition-colors">{feature.title}</h3>
  ...
  return (
    <section id="pricing" className="py-32 bg-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10 text-center max-w-4xl">
        <Badge className="bg-emerald-100 text-emerald-600 border-none px-4 py-1.5 mb-6 uppercase tracking-[0.2em] text-[10px] font-black">
          Implementação
        </Badge>
        <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">
          Instalado pelo <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-pink-600">nosso time</span>
        </h2>
        <p className="text-slate-500 max-w-2xl mx-auto text-lg mb-16">
          O AKAAI Court é implantado com suporte especializado. Entre em contato para que nossa equipe entenda o seu estúdio e configure a plataforma perfeita para vocês.
        </p>
        ...
        <div className="bg-gradient-to-br from-emerald-600 to-pink-600 rounded-3xl p-10 text-white shadow-2xl shadow-emerald-500/20">
          <h3 className="text-2xl font-black mb-3">Pronto para transformar seu estúdio?</h3>
          <p className="text-emerald-100 mb-8">
            Fale com nosso time e receba uma demonstração exclusiva do AKAAI Court para o seu estúdio.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href="https://wa.me/5511999999999?text=Olá! Tenho interesse no AKAAI Court para meu estúdio de dança."
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 bg-white text-emerald-700 hover:bg-emerald-50 font-bold h-14 px-8 rounded-2xl transition-all shadow-lg"
            >
              <Trophy className="w-5 h-5" />
...
function CTASection() {
  return (
    <section className="py-32 bg-slate-950/90 relative overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-from)_0%,_transparent_70%)] from-emerald-900/30 via-transparent to-transparent" />
      </div>
      <div className="container mx-auto px-6 relative z-10 text-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 font-bold text-sm mb-8">
            <Sparkles className="w-4 h-4" />
            14 dias grátis, sem cartão
          </div>
          <h2 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">
            Pronto para transformar <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-pink-400">seu estúdio?</span>
          </h2>
          <p className="text-slate-400 text-xl max-w-2xl mx-auto mb-12">
            Junte-se a centenas de estúdios que já modernizaram sua gestão com o AKAAI Court.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button size="lg" className="h-16 px-12 text-xl rounded-full bg-emerald-600 hover:bg-emerald-700 shadow-[0_10px_40px_-10px_rgba(139,92,246,0.6)] hover:scale-105 transition-all font-bold text-white border-none group" asChild>
              <Link href="/solutions/sports_center/register">
                Criar Conta Grátis
                <ArrowRight className="ml-2 w-6 h-6 group-hover:translate-x-1 transition-transform" />
              </Link>
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-12 pb-12 border-b border-white/5">
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              O ecossistema **akaaicore** fornece a infraestrutura robusta, modular e baseada em IA que alimenta o AKAAI Court, 
          ...
          <div className="md:col-span-3 space-y-6">
            ...
            <nav className="flex flex-col gap-3">
              ...
              <Link href="/solutions/sports_center/login" className="text-slate-500 hover:text-emerald-400 text-sm transition-colors flex items-center gap-2 group">
                <div className="w-1 h-1 rounded-full bg-emerald-500/50 group-hover:bg-emerald-400 transition-colors" />
                Login no Sistema
              </Link>
            </nav>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-4">
          <p className="text-slate-600 text-[10px] uppercase tracking-[0.2em] font-black">
            © {new Date().getFullYear()} AKAAI Court — powered by akaaicore. Todos os direitos reservados.
          </p>
