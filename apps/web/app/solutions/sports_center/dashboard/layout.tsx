...
function DanceSidebar({
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/sports_center")
  }
        ...
        {!collapsed && (
          <Link href="/solutions/sports_center" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              AKAAI <span className="text-blue-400">Court</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function DanceDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (role === 'student') {
        router.push("/solutions/sports_center/student")
        return
      }
      if (role === 'teacher') {
        router.push("/solutions/sports_center/teacher")
        return
      }
      setAuthorized(true)
      setLoading(false)
