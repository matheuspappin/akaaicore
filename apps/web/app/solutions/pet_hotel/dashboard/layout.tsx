...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/pet_hotel/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/pet_hotel/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/pet_hotel/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/pet_hotel/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/pet_hotel/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/pet_hotel/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/pet_hotel/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/pet_hotel/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/pet_hotel/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/pet_hotel/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/pet_hotel/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/pet_hotel/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/pet_hotel/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/pet_hotel/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/pet_hotel/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/pet_hotel/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/pet_hotel/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/pet_hotel" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/pet_hotel/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/pet_hotel/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/pet_hotel/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/pet_hotel/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/pet_hotel/client")
          return
        }
        setAuthorized(true)
      }
