...
function LoginContent() {
  ...
  React.useEffect(() => {
    async function checkSession() {
      ...
      if (session?.user) {
        ...
        if (userRole === 'engineer') router.push("/solutions/art_studio/engineer")
        else if (userRole === 'architect') router.push("/solutions/art_studio/architect")
        else if (userRole === 'technician' || userRole === 'teacher') router.push("/solutions/art_studio/technician")
        else if (userRole === 'student') router.push("/solutions/art_studio/client")
        else if (userRole === 'super_admin') router.push("/admin")
        else if (userRole === 'finance') router.push("/solutions/art_studio/dashboard/financeiro")
        else router.push("/solutions/art_studio/dashboard")
      }
    }
    checkSession()
  }, [router])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        else if (data.user.role === 'student') router.push("/solutions/art_studio/client")
        else if (data.user.role === 'teacher' || data.user.role === 'technician') router.push("/solutions/art_studio/technician")
        else if (data.user.role === 'engineer') router.push("/solutions/art_studio/engineer")
        else if (data.user.role === 'architect') router.push("/solutions/art_studio/architect")
        else if (data.user.role === 'seller') router.push("/seller")
        else if (data.user.role === 'finance') router.push("/solutions/art_studio/dashboard/financeiro")
        else router.push("/solutions/art_studio/dashboard")
      } else {
        toast({
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-800 via-violet-700 to-orange-700 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-30">
          ...
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-violet-900/50 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2" />
        ...
        <Link href="/solutions/art_studio" className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center shadow-2xl">
            <FireExtinguisher className="w-6 h-6 text-violet-600" />
          </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-slate-950 relative overflow-hidden">
        ...
        <div className="relative w-full max-w-md">
          <div className="lg:hidden mb-12 text-center">
            <Link href="/solutions/art_studio" className="inline-flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-violet-600 flex items-center justify-center shadow-lg shadow-violet-600/20">
                <FireExtinguisher className="w-6 h-6 text-white" />
              </div>
              <span className="text-2xl font-black text-white tracking-tighter">
                Fire<span className="text-violet-500">Control</span>
              </span>
            </Link>
          </div>
          ...
          <Card className="relative overflow-hidden bg-slate-900/90 backdrop-blur-2xl border border-white/[0.06] shadow-2xl shadow-black/60 ring-1 ring-white/[0.04]">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-px bg-gradient-to-r from-transparent via-violet-500/50 to-transparent" />
            {/* Brilho sutil nas bordas */}
            <div className="absolute -inset-px bg-gradient-to-b from-violet-500/5 via-transparent to-transparent rounded-[inherit] [mask-image:linear-gradient(black,transparent_70%)] pointer-events-none" />
            ...
            <CardContent className="px-6 pb-8">
                        ...
                        {profiles.map((p) => (
                          <button
                            ...
                            className={cn(
                              ...
                              role === p.id ? "bg-violet-500/10 text-violet-400" : "text-slate-400 hover:bg-white/5 hover:text-white"
                            )}
                          >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Link href="/forgot-password?returnTo=/solutions/art_studio/login" title="Esqueceu?" className="text-xs text-violet-500/80 hover:text-violet-400 transition-colors">
                      Esqueceu?
                    </Link>
                  </div>
                  <div className="relative">
              ...
              <div className="mt-6 pt-5 border-t border-white/5 space-y-4">
                <p className="text-center text-sm text-slate-500">
                  Não possui conta?{" "}
                  <Link href="/solutions/art_studio/register" className="text-violet-500 hover:text-violet-400 font-medium transition-colors">
                    Criar conta grátis
                  </Link>
                </p>
                <Link
