...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/beauty/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/beauty/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/beauty/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/beauty/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/beauty/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/beauty/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/beauty/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/beauty/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/beauty/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/beauty/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/beauty/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/beauty/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/beauty/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/beauty/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/beauty/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/beauty/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/beauty/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/beauty" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/beauty/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/beauty/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/beauty/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/beauty/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/beauty/client")
          return
        }
        setAuthorized(true)
      }
