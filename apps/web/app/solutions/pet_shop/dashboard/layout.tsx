...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/pet_shop/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/pet_shop/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/pet_shop/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/pet_shop/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/pet_shop/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/pet_shop/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/pet_shop/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/pet_shop/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/pet_shop/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/pet_shop/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/pet_shop/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/pet_shop/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/pet_shop/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/pet_shop/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/pet_shop/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/pet_shop/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/pet_shop/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        ...
        if (role === 'engineer') {
          router.push("/solutions/pet_shop/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/pet_shop/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/pet_shop/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/pet_shop/client")
          return
        }
        setAuthorized(true)
      }
