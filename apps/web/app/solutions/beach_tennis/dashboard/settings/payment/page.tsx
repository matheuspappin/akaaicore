// apps/web/app/solutions/beach_tennis/dashboard/settings/payment/page.tsx
"use client"
...
import {
  CreditCard, QrCode, Package, Save, Loader2, Coins, CheckCircle, XCircle, Unlink, ExternalLink,
  DollarSign
} from "lucide-react"
...
export default function PaymentSettingsPage() {
  ...
  const loadPaymentsConfig = async () => {
    ...
    try {
      ...
      // Verifica PagBank
      const { data: studioData } = await supabase.from("studios").select("pagbank_access_token").eq("id", studioId).single()
      ...
      const { data: settings } = await supabase.from("studio_settings").select("*").eq("studio_id", studioId).eq("setting_key", "pdv_credit_reais_per_unit").maybeSingle()
      ...
      <div className="p-4 md:p-8 space-y-8 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <h2 className="text-lg font-black flex items-center gap-2"><CreditCard className="w-5 h-5 text-orange-500" /> Métodos de Pagamento</h2>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                {pagbankStatus === "connected" ? (
                  <Button variant="outline" size="sm" className="w-full text-red-500 hover:text-red-600 border-red-200" onClick={handleDisconnectPagbank} disabled={pagbankLoading}>
                    {pagbankLoading ? <Loader2 className="w-3 h-3 mr-2 animate-spin" /> : <Unlink className="w-3 h-3 mr-2" />}
                    Desconectar PagBank
                  </Button>
                ) : (
                  <Button size="sm" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleConnectPagbank} disabled={pagbankLoading || !studioId}>
                    {pagbankLoading ? <Loader2 className="w-3 h-3 mr-2 animate-spin" /> : <QrCode className="w-3 h-3 mr-2" />}
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center justify-between text-base">
                  <div className="flex items-center gap-2"><CreditCard className="w-4 h-4 text-orange-500" /> Stripe (Pagamentos)</div>
                  <div className="flex items-center gap-1.5 text-xs">
              ...
              <CardContent className="space-y-3">
                <p className="text-xs text-slate-500">
                  Conecte sua conta Stripe para receber pagamentos de pacotes e assinaturas recorrentes.
                </p>
                {stripeConnectStatus?.charges_enabled ? (
                  ...
                  >
                    {stripeConnectLoading ? <Loader2 className="w-3 h-3 mr-2 animate-spin" /> : <CreditCard className="w-3 h-3 mr-2" />}
                    {stripeConnectStatus?.stripe_account_id ? "Concluir Onboarding" : "Conectar Stripe"}
                  </Button>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base"><Coins className="w-4 h-4 text-amber-500" /> Valor de Troca (PDV)</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <p className="text-xs text-slate-500">Quanto vale 1 crédito ao ser usado como pagamento na loja/balcão.</p>
          ...
          {/* Pacotes de Créditos */}
          <div className="space-y-6">
            <h2 className="text-lg font-black flex items-center gap-2"><Package className="w-5 h-5 text-orange-500" /> Pacotes de Créditos</h2>
            
            <Card>
              <CardContent className="pt-6">
                {loadingCredits ? (
                  <div className="py-8 flex justify-center"><Loader2 className="w-5 h-5 animate-spin text-orange-500" /></div>
                ) : creditPackages.length === 0 ? (
                  <p className="text-center text-sm text-slate-500 py-6">Nenhum pacote cadastrado.</p>
                ) : (
                  <div className="space-y-3 mb-6">
                    {creditPackages.map(pkg => (
                      <div key={pkg.id} className="flex items-center justify-between p-3 rounded-xl border bg-white dark:bg-white/5 transition-colors hover:border-violet-200">
                        <div>
                          <p className="font-bold text-sm">{pkg.name || `Pacote ${pkg.lessons_count} aulas`}</p>
                          <p className="text-[10px] text-slate-500 uppercase font-bold tracking-tight">
                            {pkg.lessons_count} sessões · {pkg.validity_days || 90} dias
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                          <p className="font-black text-orange-600 text-sm">R$ {Number(pkg.price).toFixed(2)}</p>
                          <Button variant="ghost" size="icon" className="text-red-400 hover:text-red-500 h-8 w-8" onClick={() => deleteCreditPackage(pkg.id)}>
                ...
                <div className="border-t pt-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Novo Pacote</p>
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <Label className="text-[11px] font-bold uppercase text-slate-500">Nome</Label>
                      <Input placeholder="Ex: Pacote Verão" className="h-9" value={newPkg.name} onChange={e => setNewPkg(p => ({ ...p, name: e.target.value }))} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <Label className="text-[11px] font-bold uppercase text-slate-500">Créditos</Label>
                        <Input type="number" className="h-9" value={newPkg.lessons_count} onChange={e => setNewPkg(p => ({ ...p, lessons_count: parseInt(e.target.value) || 1 }))} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[11px] font-bold uppercase text-slate-500">Preço (R$)</Label>
                        <Input type="number" step={0.01} className="h-9" value={newPkg.price} onChange={e => setNewPkg(p => ({ ...p, price: parseFloat(e.target.value) || 0 }))} />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <Label className="text-[11px] font-bold uppercase text-slate-500">Validade (dias)</Label>
                        <Input type="number" className="h-9" value={newPkg.validity_days} onChange={e => setNewPkg(p => ({ ...p, validity_days: parseInt(e.target.value) || 30 }))} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[11px] font-bold uppercase text-slate-500">Tipo Cobrança</Label>
                        <Select value={newPkg.billing_type} onValueChange={v => setNewPkg(p => ({ ...p, billing_type: v as "one_time" | "monthly" }))}>
                    ...
                    <Button className="w-full bg-orange-600 hover:bg-orange-700 h-10 mt-2 font-bold" onClick={addCreditPackage} disabled={!newPkg.lessons_count || !newPkg.price}>
