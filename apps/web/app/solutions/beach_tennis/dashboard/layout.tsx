...
function DanceSidebar({
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/beach_tennis")
  }
        ...
        {!collapsed && (
          <Link href="/solutions/beach_tennis" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              AKAAI <span className="text-orange-400">Court</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function DanceDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (role === 'student') {
        router.push("/solutions/beach_tennis/student")
        return
      }
      if (role === 'teacher') {
        router.push("/solutions/beach_tennis/teacher")
        return
      }
      setAuthorized(true)
      setLoading(false)
