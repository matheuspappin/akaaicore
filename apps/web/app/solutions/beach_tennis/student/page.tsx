...
const COLORS = ["border-l-pink-500", "border-l-orange-500", "border-l-indigo-500", "border-l-emerald-500"]
...
export default function StudentHome() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      await Promise.all([
        ...
        // Financeiro — busca cobranças do aluno via API BeachTennis
        (async () => {
          try {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-br from-orange-600 to-pink-600 rounded-2xl p-6 text-white relative">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10">
            <CardContent className="p-4 text-center">
              <div className="w-10 h-10 rounded-xl bg-violet-100 dark:bg-orange-600/20 flex items-center justify-center mx-auto mb-2">
      ...
      {!loading && financeiro.hasDebito && (
        <Card className={cn(
          ...
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <Link href="/solutions/beach_tennis/student/financeiro">
      ...
      {!loading && nextClass && (
        <Card className="overflow-hidden border-none shadow-md bg-gradient-to-br from-orange-600 to-pink-600 text-white">
            ...
            <div className="bg-black/10 p-3 flex justify-between items-center px-5">
              ...
              {attendanceRecord?.status === 'present' ? (
                ...
                <Button
                  ...
                  size="sm"
                  variant="secondary"
                  className="bg-white text-orange-600 hover:bg-white/90 gap-1 text-xs font-bold"
                  ...
                  <div key={cls.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                    ...
                    {isPresent ? (
                      ...
                      <Button
                        ...
                        size="sm"
                        variant="ghost"
                        className="h-8 text-xs font-bold text-orange-600 hover:text-orange-700 hover:bg-violet-50 px-3"
                        onClick={() => {
      ...
      <Card className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button type="button" variant="ghost" size="sm" className="text-orange-600 text-xs font-bold h-7" asChild>
              <Link href="/solutions/beach_tennis/student/turmas">
                Ver todas <ArrowRight className="w-3 h-3 ml-1" />
              </Link>
            </Button>
          </div>
          {loading ? (
            <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 animate-spin text-orange-600" /></div>
            ...
            <div className="space-y-2">
              {turmas.slice(0, 3).map((t: any) => (
                <div key={t.id} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 dark:bg-white/5">
                  ...
                  {t.dance_style && (
                    <Badge className="bg-violet-100 text-orange-700 dark:bg-orange-600/20 dark:text-orange-400 border-0 text-[10px] font-bold">
                      {t.dance_style}
                    </Badge>
                  )}
                </div>
      ...
      <Dialog open={isClassQrOpen} onOpenChange={setIsClassQrOpen}>
        <DialogContent className="sm:max-w-md max-w-[90vw] rounded-2xl border-none">
          ...
          <div className="flex flex-col items-center justify-center py-6 gap-6">
            <div className="bg-white p-4 rounded-3xl shadow-xl border-4 border-violet-50 dark:border-orange-900/30">
