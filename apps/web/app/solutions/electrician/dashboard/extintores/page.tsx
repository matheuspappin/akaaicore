...
const statusMap = {
  ...
  vencido: { label: "Vencido", icon: XCircle, className: "bg-red-100 text-yellow-700 dark:bg-yellow-600/20 dark:text-yellow-400" },
}
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-yellow-600 hover:bg-yellow-700 text-white font-bold rounded-xl shadow-lg shadow-yellow-600/20" disabled={!studioId}>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 py-2">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" className="flex-1 bg-yellow-600 hover:bg-yellow-700 text-white font-bold" disabled={loading}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          <Button onClick={handlePrint} className="w-full bg-yellow-600 hover:bg-yellow-700" size="lg">
...
export default function ExtintoresPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Alertas de status */}
      {counts.vencido > 0 && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-red-50 dark:bg-yellow-600/10 border border-red-200 dark:border-yellow-600/30">
        ...
        <div className="flex gap-2">
          {(["todos", "ok", "vencendo", "vencido"] as const).map((f) => (
            <Button
              ...
              size="sm"
              className={cn(filtro === f && "bg-yellow-600 hover:bg-yellow-700 border-yellow-600")}
              onClick={() => setFiltro(f)}
            >
                ...
                return (
                  <tr key={ext.id} className={cn("border-b border-slate-50 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors", i === filtered.length - 1 && "border-0")}>
                    <td className="px-4 py-3">
                      <span className="text-xs font-black text-yellow-600 bg-red-50 dark:bg-yellow-600/10 px-2 py-0.5 rounded-lg">{displayId(ext)}</span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem className="text-yellow-600"><Trash2 className="w-4 h-4 mr-2" />Remover</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </td>
                  </tr>
