...
const navItems = [
  { href: "/solutions/photographer/client", label: "Início", icon: LayoutDashboard },
  { href: "/solutions/photographer/client/extintores", label: "Meus Extintores", icon: Package },
  { href: "/solutions/photographer/client/documentos", label: "Documentos", icon: FileText },
  { href: "/solutions/photographer/client/perfil", label: "Meu Perfil", icon: UserCircle },
]
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-16 flex items-center justify-between gap-4 px-4 md:px-6 bg-slate-950/95 dark:bg-slate-950 border-b border-white/10 backdrop-blur-md supports-[backdrop-filter]:bg-slate-950/80">
        <Link
          href="/solutions/photographer/client"
          className="flex items-center gap-3 group"
        >
          <div className="w-10 h-10 rounded-xl bg-zinc-600 flex items-center justify-center shadow-lg shadow-zinc-600/25 group-hover:shadow-zinc-600/40 transition-shadow">
      ...
      <nav className="hidden md:flex bg-white dark:bg-slate-900/50 border-b border-slate-200 dark:border-white/10 px-4 md:px-6">
        <div className="flex gap-1 max-w-4xl w-full">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== "/solutions/photographer/client" &&
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "text-zinc-600 border-zinc-600 dark:text-zinc-500 dark:border-zinc-500"
                    : "text-slate-500 hover:text-zinc-600 border-transparent hover:border-zinc-600/50 dark:text-slate-400 dark:hover:text-zinc-500"
                )}
              >
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "text-zinc-600 dark:text-zinc-500 bg-red-50/50 dark:bg-zinc-600/10"
                  : "text-slate-400 hover:text-zinc-600 dark:hover:text-zinc-500"
              )}
            >
