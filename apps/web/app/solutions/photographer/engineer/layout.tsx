...
const navItems = [
  { href: "/solutions/photographer/engineer", label: "Dashboard", icon: LayoutDashboard },
  { href: "/solutions/photographer/engineer/projetos", label: "Projetos PPCI", icon: FolderKanban },
  { href: "/solutions/photographer/engineer/perfil", label: "Meu Perfil", icon: UserCircle },
]
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        <Link href="/solutions/photographer" className="flex items-center gap-2">
          ...
          {(!collapsed || isMobile) && (
            <div>
              <span className="font-black text-white tracking-tighter text-xs block">Fotógrafo(Studio)</span>
              <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">Engenheiro</span>
            </div>
          )}
        </Link>
        {!isMobile && (
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950 border-b border-white/10 flex items-center justify-between px-4 z-40">
        <div className="flex items-center gap-2">
          ...
          <span className="font-black text-white text-sm">Fotógrafo(Studio) <span className="text-zinc-400">Eng</span></span>
        </div>
