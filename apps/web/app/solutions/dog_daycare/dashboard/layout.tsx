...
function DanceSidebar({
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/dog_daycare")
  }
        ...
        {!collapsed && (
          <Link href="/solutions/dog_daycare" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              AKAAI <span className="text-orange-400">Pet</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function DanceDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (role === 'student') {
        router.push("/solutions/dog_daycare/student")
        return
      }
      if (role === 'teacher') {
        router.push("/solutions/dog_daycare/teacher")
        return
      }
      setAuthorized(true)
      setLoading(false)
