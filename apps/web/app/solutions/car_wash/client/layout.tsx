...
const navItems = [
  { href: "/solutions/car_wash/client", label: "Início", icon: LayoutDashboard },
  { href: "/solutions/car_wash/client/extintores", label: "Meus Extintores", icon: Package },
  { href: "/solutions/car_wash/client/documentos", label: "Documentos", icon: FileText },
  { href: "/solutions/car_wash/client/perfil", label: "Meu Perfil", icon: UserCircle },
]
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-16 flex items-center justify-between gap-4 px-4 md:px-6 bg-slate-950/95 dark:bg-slate-950 border-b border-white/10 backdrop-blur-md supports-[backdrop-filter]:bg-slate-950/80">
        <Link
          href="/solutions/car_wash/client"
          className="flex items-center gap-3 group"
        >
          <div className="w-10 h-10 rounded-xl bg-sky-600 flex items-center justify-center shadow-lg shadow-sky-600/25 group-hover:shadow-sky-600/40 transition-shadow">
        ...
        <button
          ...
          disabled={loggingOut}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-white/60 hover:text-sky-400 hover:bg-sky-600/10 text-xs font-bold uppercase tracking-widest transition-all disabled:opacity-50"
        >
          {loggingOut ? (
      ...
      <nav className="hidden md:flex bg-white dark:bg-slate-900/50 border-b border-slate-200 dark:border-white/10 px-4 md:px-6">
        <div className="flex gap-1 max-w-4xl w-full">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== "/solutions/car_wash/client" &&
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "text-sky-600 border-sky-600 dark:text-sky-500 dark:border-sky-500"
                    : "text-slate-500 hover:text-sky-600 border-transparent hover:border-sky-600/50 dark:text-slate-400 dark:hover:text-sky-500"
                )}
              >
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "text-sky-600 dark:text-sky-500 bg-red-50/50 dark:bg-sky-600/10"
                  : "text-slate-400 hover:text-sky-600 dark:hover:text-sky-500"
              )}
            >
