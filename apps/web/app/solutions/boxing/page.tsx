...
function Navbar() {
  ...
  const navItems = [
    ...
    { name: "App da Equipe", href: "#teacher-app" },
    { name: "Portal do Aluno", href: "#guardian-portal" },
      ...
      <div className="container mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 font-bold text-2xl tracking-tight text-white hover:opacity-80 transition-opacity">
          <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-r from-red-600 to-pink-600 rounded-lg blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
          ...
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-white/70">
            AKAAI <span className="text-red-400 font-black">Combat</span>
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="px-4 py-2 text-sm font-bold text-white/70 hover:text-red-400 transition-all relative group uppercase tracking-widest"
            >
              {item.name}
        ...
        <div className="flex items-center gap-3">
            <Button size="sm" variant="ghost" className="font-semibold text-white hover:bg-white/10 hover:text-white transition-all" asChild>
              <Link href="/solutions/boxing/login">Entrar</Link>
            </Button>
            <Button size="sm" className="hidden sm:flex rounded-full px-6 bg-red-600 hover:bg-red-700 shadow-lg shadow-red-600/20 hover:shadow-red-600/40 hover:scale-105 transition-all duration-300 font-bold text-white border-none" asChild>
              <Link href="/solutions/boxing/register">Criar Conta Grátis</Link>
            </Button>
          ...
          >
            <div className="flex flex-col p-6 space-y-4 font-semibold text-white">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                ...
                <Button type="button" className="w-full rounded-xl bg-red-600 hover:bg-red-700 text-white border-none" asChild>
                  <Link href="/solutions/boxing/register">Criar Conta</Link>
                </Button>
              </div>
            </div>
          </motion.div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            ...
            Novo: Catraca digital e QR Code no app
          </motion.div>
          ...
          >
            Matrículas, turmas, frequência de atletas, pagamentos e comunicação com alunos — tudo em um só lugar. Foque no ringue, não na burocracia.
          </motion.p>
          ...
          >
            <Button size="lg" className="h-16 px-10 text-xl rounded-full bg-gradient-to-r from-cyan-500 to-fuchsia-600 hover:from-cyan-400 hover:to-fuchsia-500 shadow-[0_0_40px_rgba(0,255,255,0.25)] hover:shadow-[0_0_60px_rgba(168,85,247,0.3)] transition-all hover:scale-105 font-bold text-white border-none group" asChild>
              <Link href="/solutions/boxing/register">
                Começar Grátis
          ...
          >
            {[
              { value: "+500", label: "Academias ativos" },
              { value: "+12k", label: "Atletas gerenciados" },
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Tudo na palma da mão da sua <span className="text-red-400">Equipe</span>
          </h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">
            Chamada digital, frequência automática por QR Code e gestão de treinos sem complicação para seus professores.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-12 gap-8 items-center max-w-6xl mx-auto">
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                    <div className="bg-white p-4 rounded-2xl shadow-sm mb-4 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-slate-800 font-bold">
                        <ClipboardList className="w-5 h-5 text-red-600" /> Chamada
                      </div>
                      <Badge className="bg-violet-100 text-red-600 border-none">Boxe Iniciante</Badge>
                    </div>
                    <div className="space-y-2 flex-1 overflow-auto">
                      {[
                        { name: "Carlos Silva", present: true },
                        { name: "Ricardo Santos", present: true },
                        { name: "Marcos Oliveira", present: false },
                        { name: "André Costa", present: true },
                        { name: "Lucas Ferreira", present: false },
                      ].map((atleta) => (
                        <div key={atleta.name} className={cn(
                          "bg-white p-3 rounded-xl shadow-sm flex items-center justify-between border-l-4",
                          atleta.present ? "border-l-emerald-500" : "border-l-red-400"
                        )}>
                          <div className="flex items-center gap-2">
                            <div className={cn(
                              "w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white",
                              atleta.present ? "bg-emerald-500" : "bg-slate-300"
                            )}>
                              {atleta.name[0]}
                            </div>
                            <span className="text-sm font-semibold text-slate-700">{atleta.name}</span>
                          </div>
                          {atleta.present
                    ...
                    <Button size="sm" className="mt-4 bg-red-600 text-white w-full rounded-xl">
                      Salvar Chamada
                    </Button>
                  </motion.div>
                )}
                ...
                {activeTab === 'turma' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="font-bold text-lg text-slate-800">Olá, Treinador 👋</h3>
                    ...
                    <div className="space-y-3">
                      {[
                        { name: "Boxe Iniciante", time: "09:00", students: 12, color: "border-l-red-400" },
                        { name: "Muay Thai", time: "11:00", students: 8, color: "border-l-amber-400" },
                        { name: "Sparring Avançado", time: "19:00", students: 15, color: "border-l-indigo-400" },
                      ].map((turma) => (
                        <div key={turma.name} className={cn("bg-white p-4 rounded-2xl shadow-sm border-l-4", turma.color)}>
                          ...
                          <div className="flex items-center justify-between mt-1">
                            ...
                            <div className="flex items-center gap-1 text-xs text-slate-500">
                              <Users className="w-3 h-3" /> {turma.students} atletas
                            </div>
                          </div>
          ...
          <div className="md:col-span-7 space-y-6">
            {[
              { id: 'chamada', title: 'Chamada Digital', desc: 'Frequência em segundos, sem papel. Histórico completo por atleta.', icon: ClipboardList },
              { id: 'turma', title: 'Agenda de Turmas', desc: 'Visualize todas as turmas do dia com horários e número de atletas.', icon: Calendar },
              { id: 'qrcode', title: 'Check-in por QR Code', desc: 'Seus alunos fazem check-in autônomo na entrada da academia.', icon: Smartphone }
            ].map((feature) => (
              <div
                key={feature.id}
...
function StudentPortalSection() {
  return (
    <section id="guardian-portal" className="py-24 bg-slate-950/90 text-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <Badge className="bg-red-100 text-red-600 border-none px-4 py-1 mb-4 uppercase tracking-widest text-[10px] font-black">Portal do Aluno</Badge>
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Experiência VIP para seus <span className="text-red-400">Alunos</span>
          </h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">
            Seus atletas acompanham frequência, recebem comunicados e pagam mensalidades direto pelo celular.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-12 gap-8 items-center max-w-6xl mx-auto">
          ...
          <div className="md:col-span-5 flex justify-center">
            <div className="relative w-[300px] h-[600px] bg-slate-950 rounded-[3rem] border-8 border-slate-800 shadow-2xl overflow-hidden">
              ...
              <div className="w-full h-full bg-slate-50 pt-10 pb-4 px-4 flex flex-col">
                {/* Header portal */}
                <div className="bg-red-700 p-4 rounded-2xl shadow-xl border border-red-600 mb-4">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-sm text-white">AKAAI Combat — Portal</span>
                    <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center font-bold text-xs text-white">CS</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div className="bg-white p-3 rounded-xl text-center shadow-sm">
                    <p className="text-2xl font-black text-red-600">92%</p>
                  ...
                  <div className="bg-white p-3 rounded-xl text-center shadow-sm">
                    ...
                    <p className="text-[10px] font-bold uppercase text-slate-400">Treinos no mês</p>
                  </div>
                </div>
                {/* Comunicados */}
                <div className="bg-white p-3 rounded-xl shadow-sm flex-1 overflow-auto">
                  ...
                  {[
                    { text: "Graduação de Faixas: 15/06 às 19h", icon: Star, color: "text-red-500" },
                    { text: "Treino extra: Sábado 10h", icon: Calendar, color: "text-red-600" },
                  ].map((item, i) => (
          ...
          <div className="md:col-span-7 space-y-6">
            {[
              { title: 'Frequência em Tempo Real', desc: 'Alunos acompanham seu progresso e frequência em cada treino.', icon: TrendingUp, color: "bg-red-600" },
              { title: 'Pagamentos Online', desc: 'Boleto, cartão ou Pix. Histórico completo e renovação simplificada.', icon: CreditCard, color: "bg-red-700" },
              { title: 'Comunicados & Eventos', desc: 'Avisos de treinos extras, graduações e feriados diretamente no app.', icon: MessageSquare, color: "bg-red-800" },
            ].map((feature) => (
              <div key={feature.title} className="p-6 rounded-2xl bg-slate-800 border border-red-500/20 shadow-lg">
                <div className="flex gap-4">
...
function ManagementDashboard() {
  const stats = [
    { label: "Atletas Ativos", value: "124", icon: Users, color: "text-red-500", trend: "+8 este mês" },
    { label: "Faturamento", value: "R$ 14.800", icon: DollarSign, color: "text-emerald-500", trend: "+12% vs. mês anterior" },
    { label: "Taxa de Retenção", value: "94%", icon: Heart, color: "text-red-500", trend: "Acima da média" },
    { label: "Turmas Ativas", value: "18", icon: Calendar, color: "text-indigo-500", trend: "3 novas turmas" },
  ]

  const modules = [
    { label: "Turmas", icon: GraduationCap, color: "bg-red-600", shadow: "shadow-red-500/30", description: "Crie e gerencie turmas por nível e horário." },
    { label: "Atletas", icon: Users, color: "bg-red-700", shadow: "shadow-red-500/30", description: "Cadastro completo com histórico de frequência." },
    ...
    { label: "Eventos", icon: Star, color: "bg-amber-500", shadow: "shadow-amber-500/30", description: "Gestão de competições e eventos internos." },
    { label: "Treinadores", icon: Heart, color: "bg-rose-600", shadow: "shadow-rose-500/30", description: "Agenda, pagamentos e avaliações de professores." },
  ...
  return (
    <section id="management" className="py-24 bg-slate-950/90 relative overflow-hidden">
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
        <div className="absolute top-[20%] right-[-5%] w-[400px] h-[400px] bg-red-600/5 rounded-full blur-[100px]" />
        <div className="absolute bottom-[10%] left-[-5%] w-[400px] h-[400px] bg-red-600/5 rounded-full blur-[100px]" />
      </div>
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <Badge className="bg-red-500/20 text-red-400 border-red-500/30 px-4 py-1 mb-4 uppercase tracking-widest text-[10px] font-black">Painel Administrativo</Badge>
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Controle total da sua <span className="text-red-400">Academia</span>
          </h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">
                  ...
                  <Tooltip key={i}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <span className="text-[10px] sm:text-xs font-bold text-slate-600 text-center uppercase tracking-tighter group-hover:text-red-500 transition-colors">
                          {mod.label}
                        </span>
                      </motion.div>
                    </TooltipTrigger>
...
function FeatureGrid() {
  const features = [
    { title: "Controle de Mensalidades", description: "Cobranças automáticas com boleto, cartão ou Pix. Inadimplência em destaque.", icon: CreditCard, color: "from-red-500 to-red-400" },
    ...
    { title: "Captação de Alunos", description: "Formulário de matrícula online e funil de leads integrado.", icon: UserPlus, color: "from-red-600 to-red-400" },
    { title: "Multi-Modalidades", description: "Boxe, Muay Thai, Kickboxing, Jiu-Jitsu e muito mais.", icon: Trophy, color: "from-amber-500 to-orange-400" },
  ]
  ...
  return (
    <section id="features" className="py-32 bg-slate-50 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-20">
        ...
        <div className="absolute bottom-[10%] right-[5%] w-96 h-96 bg-red-400 rounded-full blur-[128px]" />
      </div>
        ...
        <div className="text-center mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">
              Tudo em um <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-red-400">só lugar</span>
            </h2>
            <p className="text-slate-500 max-w-2xl mx-auto text-lg leading-relaxed">
              A plataforma mais completa para modernizar a gestão da sua academia e encantar seus alunos.
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {features.map((feature, i) => (
            ...
            >
              ...
              <h3 className="text-xl font-black text-slate-900 mb-3 group-hover:text-red-600 transition-colors">{feature.title}</h3>
...
function PricingSection() {
  const highlights = [
    ...
    { icon: "📱", label: "Treinamento completo", desc: "Capacitação da sua equipe e recepção" },
    { icon: "💬", label: "Suporte dedicado", desc: "Atendimento direto com especialistas em gestão esportiva" },
    { icon: "🎯", label: "Personalização total", desc: "Sistema configurado para o fluxo da sua academia" },
  ]

  return (
    <section id="pricing" className="py-32 bg-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10 text-center max-w-4xl">
        <Badge className="bg-red-100 text-red-600 border-none px-4 py-1.5 mb-6 uppercase tracking-[0.2em] text-[10px] font-black">
          Implementação
        </Badge>
        <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">
          Instalado pelo <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-red-400">nosso time</span>
        </h2>
        <p className="text-slate-500 max-w-2xl mx-auto text-lg mb-16">
          O AKAAI Combat é implantado com suporte especializado. Entre em contato para que nossa equipe entenda a sua academia e configure a plataforma perfeita para vocês.
        </p>
        ...
        <div className="bg-gradient-to-br from-red-600 to-red-800 rounded-3xl p-10 text-white shadow-2xl shadow-red-500/20">
          <h3 className="text-2xl font-black mb-3">Pronto para transformar sua academia?</h3>
          <p className="text-red-100 mb-8">
            Fale com nosso time e receba uma demonstração exclusiva do AKAAI Combat para o seu negócio.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href="https://wa.me/5511999999999?text=Olá! Tenho interesse no AKAAI Combat para minha academia de boxe."
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 bg-white text-red-700 hover:bg-red-50 font-bold h-14 px-8 rounded-2xl transition-all shadow-lg"
            >
              <Dumbbell className="w-5 h-5" />
    ...
    <section className="py-32 bg-slate-950/90 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 text-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-red-500/30 bg-red-500/10 text-red-400 font-bold text-sm mb-8">
            <Sparkles className="w-4 h-4" />
            14 dias grátis, sem cartão
          </div>
          <h2 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">
            Pronto para transformar <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-red-600">sua academia?</span>
          </h2>
          <p className="text-slate-400 text-xl max-w-2xl mx-auto mb-12">
            Junte-se a centenas de academias que já modernizaram sua gestão com o AKAAI Combat.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button size="lg" className="h-16 px-12 text-xl rounded-full bg-red-600 hover:bg-red-700 shadow-[0_10px_40px_-10px_rgba(239,68,68,0.6)] hover:scale-105 transition-all font-bold text-white border-none group" asChild>
              <Link href="/solutions/boxing/register">
                Criar Conta Grátis
                <ArrowRight className="ml-2 w-6 h-6 group-hover:translate-x-1 transition-transform" />
              </Link>
            ...
            <Button size="lg" variant="outline" className="h-16 px-12 text-xl rounded-full border-2 border-white/10 hover:bg-white/10 hover:text-white font-bold text-slate-300 bg-transparent" asChild>
              <Link href="/solutions/boxing/login">Já tenho conta</Link>
            </Button>
          </div>
        </motion.div>
      </div>
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-12 pb-12 border-b border-white/5">
          <div className="md:col-span-5 space-y-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed max-w-sm">
              A plataforma definitiva para gestão de academias de artes marciais. 
              Foque no treino e na evolução dos seus atletas, nós cuidamos da gestão.
            </p>
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              O ecossistema **akaaicore** fornece a infraestrutura robusta, modular e baseada em IA que alimenta o AKAAI Combat, 
          ...
          <div className="md:col-span-3 space-y-6">
            ...
            <nav className="flex flex-col gap-3">
              ...
              <Link href="/solutions/boxing/login" className="text-slate-500 hover:text-red-400 text-sm transition-colors flex items-center gap-2 group">
                <div className="w-1 h-1 rounded-full bg-red-500/50 group-hover:bg-red-400 transition-colors" />
                Login no Sistema
              </Link>
            </nav>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-4">
          <p className="text-slate-600 text-[10px] uppercase tracking-[0.2em] font-black">
            © {new Date().getFullYear()} AKAAI Combat — powered by akaaicore. Todos os direitos reservados.
          </p>
