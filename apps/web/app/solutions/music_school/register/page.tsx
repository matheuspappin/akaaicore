...
function RegisterContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        toast({ title: "Conta criada com sucesso!", description: inviteStudioName ? `Bem-vindo ao ${inviteStudioName}!` : "Bem-vindo ao EscoladeMúsica!" })
        await new Promise(resolve => setTimeout(resolve, 400))
        if (role === 'student') router.push("/solutions/music_school/student")
        else if (role === 'teacher') router.push("/solutions/music_school/teacher")
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      <Link href="/solutions/music_school" className="absolute top-6 left-6 z-50 flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-800 via-violet-700 to-pink-700 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="bg-black/20 backdrop-blur-xl p-8 rounded-[2rem] border border-white/10">
          ...
          <p className="text-sm text-white/80 leading-relaxed font-medium">
            O <span className="text-white font-bold">EscoladeMúsica</span> transformou minha gestão. Antes eu passava horas com planilhas, agora tudo é automático.
          </p>
            ...
            <CardContent className="px-0 pt-4">
              ...
              <div className="mt-8 text-center">
                <p className="text-sm text-slate-400 font-medium">
                  Já tem uma conta?{" "}
                  <Link href="/solutions/music_school/login" className="text-violet-500 hover:text-violet-400 font-bold underline underline-offset-4">
                    Fazer Login
                  </Link>
                </p>
              </div>
