...
function DanceSidebar({
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/music_school")
  }
        ...
        {!collapsed && (
          <Link href="/solutions/music_school" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              AKAAI <span className="text-violet-400">Music</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function DanceDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (role === 'student') {
        router.push("/solutions/music_school/student")
        return
      }
      if (role === 'teacher') {
        router.push("/solutions/music_school/teacher")
        return
      }
      setAuthorized(true)
      setLoading(false)
