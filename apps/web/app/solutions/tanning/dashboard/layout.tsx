...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/tanning/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/tanning/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/tanning/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/tanning/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/tanning/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/tanning/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/tanning/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/tanning/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/tanning/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/tanning/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/tanning/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/tanning/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/tanning/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/tanning/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/tanning/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/tanning/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/tanning/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/tanning" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/tanning/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/tanning/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/tanning/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/tanning/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/tanning/client")
          return
        }
        setAuthorized(true)
      }
