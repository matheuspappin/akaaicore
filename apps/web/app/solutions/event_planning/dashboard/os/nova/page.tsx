...
const prioridadeConfig = [
  {
    ...
    color: "border-fuchsia-500 bg-red-50 dark:bg-fuchsia-600/10 text-fuchsia-700 dark:text-fuchsia-400",
    selected: "border-fuchsia-600 bg-fuchsia-600 text-white",
    icon: AlertCircle,
  },
  {
    value: "alta",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess(true)
        setTimeout(() => {
          router.push("/solutions/event_planning/dashboard/os")
        }, 1500)
      }
    } finally {
      setLoading(false)
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Card className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 shadow-sm">
        <CardHeader className="pb-4">
          <CardTitle className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Wrench className="w-4 h-4 text-fuchsia-600" />
            Dados do Serviço
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          <div>
            <Label className="text-sm font-bold text-slate-700 dark:text-slate-300">
              Tipo de Serviço <span className="text-fuchsia-500">*</span>
            </Label>
          ...
          <div>
            ...
            {loadingData ? (
              ...
              <div className="mt-1.5 p-3 rounded-xl border border-dashed border-slate-300 dark:border-white/10 text-sm text-slate-400 text-center">
                ...
                <Link href="/solutions/event_planning/dashboard/clientes" className="text-fuchsia-600 hover:underline font-bold">
                  Cadastrar cliente
                </Link>
              </div>
            ) : (
      ...
      <Card className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 shadow-sm">
        <CardHeader className="pb-4">
          <CardTitle className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <AlertCircle className="w-4 h-4 text-fuchsia-600" />
            Prioridade
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <div className="flex gap-3 pb-8">
        <Link href="/solutions/event_planning/dashboard/os" className="flex-1">
          <Button variant="outline" className="w-full h-12 font-bold rounded-xl">
            Cancelar
          </Button>
        </Link>
        <Button
          className="flex-1 h-12 bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-bold rounded-xl shadow-lg shadow-fuchsia-600/20"
