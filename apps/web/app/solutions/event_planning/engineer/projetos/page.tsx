...
const BASE_PATH = "/solutions/event_planning/engineer/projetos"
...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  in_progress:       { label: "Em Andamento",       color: "bg-red-100 text-fuchsia-700 border-red-200",      icon: HardHat },
...
function ProjectCard({
  ...
  return (
    <Card className="hover:shadow-md transition-all border-l-4 border-l-fuchsia-600">
      <CardContent className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4 flex-1 min-w-0">
            <div className="w-11 h-11 rounded-xl bg-red-50 dark:bg-fuchsia-900/20 flex items-center justify-center flex-shrink-0">
              <Flame className="w-5 h-5 text-fuchsia-600" />
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge className="bg-red-100 text-fuchsia-700 border-red-200 text-[10px] font-bold uppercase">
                  PPCI
                </Badge>
              ...
              {progress !== null && (
                <div className="mt-2 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden w-full max-w-xs">
                  <div
                    className="h-full bg-fuchsia-600 rounded-full transition-all"
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {isPending && (
              <>
                <Button
                  size="sm"
                  variant="outline"
                  className="border-red-200 text-fuchsia-600 hover:bg-red-50"
            ...
            {!isPending && (
              <Link href={`${BASE_PATH}/${project.id}`}>
                <Button size="sm" variant="ghost" className="text-slate-400 hover:text-fuchsia-600">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Em Andamento",      value: active.length,  color: "text-fuchsia-600",   bg: "bg-red-50 dark:bg-fuchsia-900/10",     border: "border-red-200 dark:border-fuchsia-800" },
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="w-full sm:w-auto">
          ...
          <TabsTrigger value="active" className="gap-1.5">
            ...
            Em Andamento
            {active.length > 0 && (
              <Badge className="bg-fuchsia-600 text-white text-[10px] ml-1 h-4 min-w-[16px] px-1">
                {active.length}
              </Badge>
            )}
          </TabsTrigger>
              ...
              {tabProjects[tab].length === 0 ? (
                <Card className="border-dashed border-2 bg-slate-50 dark:bg-slate-900/20">
                  <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                    <div className="w-14 h-14 rounded-full bg-red-100 dark:bg-fuchsia-900/20 flex items-center justify-center mb-3">
      ...
      <Dialog open={!!acceptTarget} onOpenChange={(open) => !open && setAcceptTarget(null)}>
        <DialogContent>
          ...
          {acceptTarget && (
            <div className="rounded-xl bg-slate-50 dark:bg-slate-900 p-4 space-y-2 text-sm border border-red-100 dark:border-fuchsia-900/30">
              ...
              <Badge className="bg-red-100 text-fuchsia-700 text-xs">Projeto PPCI</Badge>
      ...
      <Dialog open={!!rejectTarget} onOpenChange={(open) => { if (!open) { setRejectTarget(null); setRejectReason("") } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ThumbsDown className="w-5 h-5 text-fuchsia-600" />
