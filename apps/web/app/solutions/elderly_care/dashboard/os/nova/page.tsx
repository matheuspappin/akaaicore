...
const prioridadeConfig = [
  {
    ...
    color: "border-teal-500 bg-red-50 dark:bg-teal-600/10 text-teal-700 dark:text-teal-400",
    selected: "border-teal-600 bg-teal-600 text-white",
    icon: AlertCircle,
  },
  {
    value: "alta",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess(true)
        setTimeout(() => {
          router.push("/solutions/elderly_care/dashboard/os")
        }, 1500)
      }
    } finally {
      setLoading(false)
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <CardContent className="space-y-5">
          ...
          <div>
            <Label className="text-sm font-bold text-slate-700 dark:text-slate-300">
              Tipo de Serviço <span className="text-teal-500">*</span>
            </Label>
          ...
          <div>
            ...
            {loadingData ? (
              ...
              <div className="mt-1.5 p-3 rounded-xl border border-dashed border-slate-300 dark:border-white/10 text-sm text-slate-400 text-center">
                ...
                <Link href="/solutions/elderly_care/dashboard/clientes" className="text-teal-600 hover:underline font-bold">
                  Cadastrar cliente
                </Link>
              </div>
            ) : (
      ...
      <div className="flex gap-3 pb-8">
        <Link href="/solutions/elderly_care/dashboard/os" className="flex-1">
          <Button variant="outline" className="w-full h-12 font-bold rounded-xl">
            Cancelar
          </Button>
        </Link>
        <Button
          className="flex-1 h-12 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl shadow-lg shadow-teal-600/20"
