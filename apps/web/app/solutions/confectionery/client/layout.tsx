...
const navItems = [
  { href: "/solutions/confectionery/client", label: "Início", icon: LayoutDashboard },
  { href: "/solutions/confectionery/client/extintores", label: "Meus Extintores", icon: Package },
  { href: "/solutions/confectionery/client/documentos", label: "Documentos", icon: FileText },
  { href: "/solutions/confectionery/client/perfil", label: "Meu Perfil", icon: UserCircle },
]
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-16 flex items-center justify-between gap-4 px-4 md:px-6 bg-slate-950/95 dark:bg-slate-950 border-b border-white/10 backdrop-blur-md supports-[backdrop-filter]:bg-slate-950/80">
        <Link
          href="/solutions/confectionery/client"
          className="flex items-center gap-3 group"
        >
          <div className="w-10 h-10 rounded-xl bg-pink-600 flex items-center justify-center shadow-lg shadow-pink-600/25 group-hover:shadow-pink-600/40 transition-shadow">
        ...
        <button
          ...
          disabled={loggingOut}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-white/60 hover:text-pink-400 hover:bg-pink-600/10 text-xs font-bold uppercase tracking-widest transition-all disabled:opacity-50"
        >
          {loggingOut ? (
      ...
      <nav className="hidden md:flex bg-white dark:bg-slate-900/50 border-b border-slate-200 dark:border-white/10 px-4 md:px-6">
        <div className="flex gap-1 max-w-4xl w-full">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== "/solutions/confectionery/client" &&
              ...
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ...
                    : "text-slate-500 hover:text-pink-600 border-transparent hover:border-pink-600/50 dark:text-slate-400 dark:hover:text-pink-500"
                )}
              >
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "text-pink-600 dark:text-pink-500 bg-red-50/50 dark:bg-pink-600/10"
                  : "text-slate-400 hover:text-pink-600 dark:hover:text-pink-500"
              )}
            >
