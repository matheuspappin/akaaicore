...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/clinic_vet/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/clinic_vet/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/clinic_vet/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/clinic_vet/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/clinic_vet/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/clinic_vet/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/clinic_vet/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/clinic_vet/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/clinic_vet/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/clinic_vet/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/clinic_vet/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/clinic_vet/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/clinic_vet/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/clinic_vet/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/clinic_vet/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/clinic_vet/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/clinic_vet/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/clinic_vet" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/clinic_vet/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/clinic_vet/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/clinic_vet/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/clinic_vet/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/clinic_vet/client")
          return
        }
        setAuthorized(true)
      }
