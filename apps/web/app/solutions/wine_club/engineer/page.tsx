...
export default function FireEngineerDashboard() {
  ...
  const statCards = [
    ...
    { label: "Em Andamento", value: stats.active, color: "text-rose-600", icon: HardHat },
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Link href="/solutions/wine_club/engineer/projetos">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/solutions/wine_club/engineer/projetos" className="text-sm font-bold text-rose-600 hover:text-rose-700 hover:underline">
            Ver todos
          </Link>
        </div>
        ...
        {projects.length === 0 ? (
          ...
          <div className="grid gap-3">
            {projects.map((project: any) => {
              ...
              return (
                <Link
                  href={isPending ? "/solutions/wine_club/engineer/projetos" : `/solutions/wine_club/engineer/projetos/${project.id}`}
                  key={project.id}
                >
                  <Card className="hover:shadow-md transition-all cursor-pointer group border-l-4 border-l-rose-600">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-start gap-3">
                        <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0", isPPCI ? "bg-red-50" : "bg-orange-50")}>
                          {isPPCI ? <Flame className="w-5 h-5 text-rose-600" /> : <HardHat className="w-5 h-5 text-orange-600" />}
                        </div>
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isPPCI && <Badge className="bg-red-100 text-rose-700 text-[10px]">PPCI</Badge>}
                          </div>
