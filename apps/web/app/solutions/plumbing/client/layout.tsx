...
const navItems = [
  { href: "/solutions/plumbing/client", label: "Início", icon: LayoutDashboard },
  { href: "/solutions/plumbing/client/extintores", label: "Meus Extintores", icon: Package },
  { href: "/solutions/plumbing/client/documentos", label: "Documentos", icon: FileText },
  { href: "/solutions/plumbing/client/perfil", label: "Meu Perfil", icon: UserCircle },
]
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-16 flex items-center justify-between gap-4 px-4 md:px-6 bg-slate-950/95 dark:bg-slate-950 border-b border-white/10 backdrop-blur-md supports-[backdrop-filter]:bg-slate-950/80">
        <Link
          href="/solutions/plumbing/client"
          className="flex items-center gap-3 group"
        >
          <div className="w-10 h-10 rounded-xl bg-cyan-600 flex items-center justify-center shadow-lg shadow-cyan-600/25 group-hover:shadow-cyan-600/40 transition-shadow">
        ...
        <button
          ...
          disabled={loggingOut}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-white/60 hover:text-cyan-400 hover:bg-cyan-600/10 text-xs font-bold uppercase tracking-widest transition-all disabled:opacity-50"
        >
          {loggingOut ? (
      ...
      <nav className="hidden md:flex bg-white dark:bg-slate-900/50 border-b border-slate-200 dark:border-white/10 px-4 md:px-6">
        <div className="flex gap-1 max-w-4xl w-full">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== "/solutions/plumbing/client" &&
              ...
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ...
                    : "text-slate-500 hover:text-cyan-600 border-transparent hover:border-cyan-600/50 dark:text-slate-400 dark:hover:text-cyan-500"
                )}
              >
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "text-cyan-600 dark:text-cyan-500 bg-red-50/50 dark:bg-cyan-600/10"
                  : "text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-500"
              )}
            >
