...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/vet/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/vet/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/vet/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/vet/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/vet/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/vet/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/vet/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/vet/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/vet/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/vet/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/vet/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/vet/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/vet/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/vet/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/vet/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/vet/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/vet/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/vet" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/vet/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/vet/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/vet/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/vet/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/vet/client")
          return
        }
        setAuthorized(true)
      }
