...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/law/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/law/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/law/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/law/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/law/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/law/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/law/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/law/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/law/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/law/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/law/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/law/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/law/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/law/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/law/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/law/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/law/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/law" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/law/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/law/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/law/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/law/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/law/client")
          return
        }
        setAuthorized(true)
      }
