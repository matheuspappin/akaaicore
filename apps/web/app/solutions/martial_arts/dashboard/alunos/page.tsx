...
function InviteCodeDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const getInviteLink = () => {
    ...
    return `${origin}/solutions/martial_arts/register?role=student&code=${code}`
  }

  const getMatriculaLink = () => {
    const origin = typeof window !== "undefined" ? window.location.origin : ""
    return `${origin}/solutions/martial_arts/matricula/${code}`
  }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {loading ? (
          ...
          <div className="space-y-4 pt-1">
            ...
            {/* Link direto */}
            <div className="rounded-xl bg-violet-50 dark:bg-red-600/10 border border-violet-200 dark:border-red-500/20 p-4 space-y-3">
              <div>
                <p className="text-[10px] font-black text-red-600 uppercase tracking-widest mb-1">Link de Cadastro Direto ✨ Recomendado</p>
              ...
              <div className="flex gap-2">
                <Button
                  className={`flex-1 font-bold rounded-xl text-sm transition-all ${copiedLink ? "bg-emerald-600 hover:bg-emerald-700" : "bg-red-600 hover:bg-red-700"} text-white`}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button
            ...
            className="font-bold rounded-xl border-violet-200 text-red-600 hover:bg-violet-50"
          >
            ...
            {!search && (
              <div className="flex gap-3 flex-wrap justify-center">
                ...
                <Button
                  variant="outline"
                  onClick={() => setIsInviteOpen(true)}
                  className="border-violet-200 text-red-600 hover:bg-violet-50 font-bold rounded-xl"
                >
                  <Link2 className="w-4 h-4 mr-2" />
            ...
            <Card key={aluno.id ?? `aluno-${idx}`} className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 hover:shadow-md transition-shadow">
              <CardContent className="p-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-red-600/10 flex items-center justify-center font-black text-red-600 text-lg flex-shrink-0">
