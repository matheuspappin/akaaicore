// apps/web/app/solutions/martial_arts/dashboard/configuracoes/page.tsx
"use client"
...
export default function DanceConfiguracoesPage() {
  ...
  const statusIcon = (s: "connected" | "disconnected" | "connecting" | "error") => {
    ...
    if (s === "connecting") return <Loader2 className="w-4 h-4 animate-spin text-red-500" />
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      {activeTab === "perfil" && (
        <div className="space-y-4 max-w-xl">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><User className="w-4 h-4 text-red-500" /> Informações Pessoais</CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <Button className="w-full bg-red-600 hover:bg-red-700" onClick={saveProfile} disabled={saving}>
      ...
      {activeTab === "estudio" && (
        <div className="space-y-4 max-w-xl">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><Building2 className="w-4 h-4 text-red-500" /> Dados do Estúdio</CardTitle>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><Music className="w-4 h-4 text-red-500" /> Modalidades Oferecidas</CardTitle>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      studio.modalities.includes(mod)
                        ...
                        : "bg-white dark:bg-white/5 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:border-red-400"
                    )}
                  >
                    {mod}
                  </button>
          ...
          <Button className="w-full bg-red-600 hover:bg-red-700" onClick={saveStudio} disabled={saving}>
            {saving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
      ...
      {activeTab === "portais" && (
        <div className="space-y-4 max-w-xl">
          {[
            { label: "Portal do Aluno", path: "/solutions/martial_arts/student", icon: Users, color: "text-blue-500", desc: "Acesso de alunos para ver turmas, créditos e check-in" },
            { label: "Portal do Professor", path: "/solutions/martial_arts/teacher", icon: GraduationCap, color: "text-emerald-500", desc: "Acesso de professores para chamadas e turmas" },
            { label: "Registro (Novo Admin)", path: "/solutions/martial_arts/register", icon: Building2, color: "text-red-500", desc: "Página de cadastro de novo estúdio/admin" },
            { label: "Landing Page Pública", path: "/solutions/martial_arts", icon: ExternalLink, color: "text-orange-500", desc: "Página de apresentação pública do sistema" },
          ].map(portal => {
          ...
          {studio.slug && (
            <Card className="border-violet-200 dark:border-red-800/40 bg-violet-50/50 dark:bg-red-900/10">
              <CardHeader>
              ...
              <CardContent>
                <div className="flex items-center gap-2 p-3 rounded-xl bg-white dark:bg-white/5 border border-violet-200 dark:border-red-800/40">
                  <code className="flex-1 text-sm font-mono text-red-700 dark:text-red-300 truncate">
                    {origin}/s/{studio.slug}
                  </code>
                  <Button size="sm" variant="ghost" className="text-red-600" onClick={() => copyLink(`${origin}/s/${studio.slug}`)}>
                    {copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                  </Button>
                </div>
                <p className="text-xs text-slate-400 mt-2">Código do estúdio: <strong className="text-red-500">{studio.slug}</strong></p>
              </CardContent>
            </Card>
          )}
        </div>
      ...
      {activeTab === "notificacoes" && (
        <div className="space-y-4 max-w-xl">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><Bell className="w-4 h-4 text-red-500" /> Notificações Automáticas</CardTitle>
      ...
      {activeTab === "aparencia" && (
        <div className="space-y-4 max-w-xl">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><Palette className="w-4 h-4 text-red-500" /> Tema e Idioma</CardTitle>
            </CardHeader>
                  ...
                  {(["light", "dark", "system"] as const).map(t => (
                    <button
                      ...
                      className={cn(
                        ...
                        theme === t ? "bg-red-600 text-white border-red-600" : "bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300"
                      )}
                    >
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(["pt", "en"] as const).map(l => (
                    <button
                      ...
                      className={cn(
                        "p-3 rounded-xl border text-sm font-bold transition-all",
                        lang === l ? "bg-red-600 text-white border-red-600" : "bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300"
                      )}
                    >
      ...
      {activeTab === "seguranca" && (
        <div className="space-y-4 max-w-xl">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><Shield className="w-4 h-4 text-red-500" /> Alterar Senha</CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <Button className="w-full bg-red-600 hover:bg-red-700" onClick={changePassword} disabled={changingPw || !passwords.next}>
