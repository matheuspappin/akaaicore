...
function DanceSidebar({
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/martial_arts")
  }
        ...
        {!collapsed && (
          <Link href="/solutions/martial_arts" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              AKAAI <span className="text-red-400">Combat</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function DanceDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (role === 'student') {
        router.push("/solutions/martial_arts/student")
        return
      }
      if (role === 'teacher') {
        router.push("/solutions/martial_arts/teacher")
        return
      }
      setAuthorized(true)
      setLoading(false)
