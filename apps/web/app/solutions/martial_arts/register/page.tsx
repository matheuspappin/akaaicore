...
function RegisterContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        toast({ title: "Conta criada com sucesso!", description: inviteStudioName ? `Bem-vindo ao ${inviteStudioName}!` : "Bem-vindo ao ArtesMarciais!" })
        await new Promise(resolve => setTimeout(resolve, 400))
        if (role === 'student') router.push("/solutions/martial_arts/student")
        else if (role === 'teacher') router.push("/solutions/martial_arts/teacher")
        else router.push("/solutions/martial_arts/dashboard")
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      <Link href="/solutions/martial_arts" className="absolute top-6 left-6 z-50 flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
        <ArrowLeft className="w-4 h-4" /> Voltar
      </Link>

      {/* Left Side */}
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-red-800 via-red-700 to-pink-700 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <Link href="/solutions/martial_arts" className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center shadow-2xl">
            <Music className="w-6 h-6 text-red-600" />
          </div>
          <span className="text-2xl font-black text-white tracking-tighter">
            AKAAI <span className="text-violet-200">Combat</span>
          </span>
        </Link>
        ...
        <div className="bg-black/20 backdrop-blur-xl p-8 rounded-[2rem] border border-white/10">
          <div className="flex items-center gap-4 mb-4">
            <div className="flex -space-x-3">
              {[1, 2, 3].map(i => (
                <div key={i} className="w-10 h-10 rounded-full bg-slate-800 border-2 border-red-500 flex items-center justify-center text-xs font-black text-white">
          ...
          <p className="text-sm text-white/80 leading-relaxed font-medium">
            O <span className="text-white font-bold">ArtesMarciais</span> transformou minha gestão. Antes eu passava horas com planilhas, agora tudo é automático.
          </p>
          ...
          <Card className="rounded-2xl border border-white/10 py-6 px-6 md:px-8 bg-slate-900 shadow-2xl shadow-black/50">
            ...
            <CardContent className="px-0 pt-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                  {[
                    ...
                    >
                      ...
                      <span className={cn("text-[10px] opacity-70", role === r.id ? "text-red-400/80" : "text-slate-500")}>{r.desc}</span>
                    </button>
                  ))}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <Button
                  type="submit"
                  className="w-full bg-red-600 hover:bg-red-700 text-white font-black h-16 rounded-2xl text-lg shadow-xl shadow-red-600/20 hover:scale-[1.02] transition-all"
              ...
              <div className="mt-8 text-center">
                <p className="text-sm text-slate-400 font-medium">
                  Já tem uma conta?{" "}
                  <Link href="/solutions/martial_arts/login" className="text-red-500 hover:text-red-400 font-bold underline underline-offset-4">
                    Fazer Login
                  </Link>
                </p>
              </div>
