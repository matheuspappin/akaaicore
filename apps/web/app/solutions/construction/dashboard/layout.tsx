...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/construction/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/construction/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/construction/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/construction/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/construction/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/construction/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/construction/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/construction/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/construction/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/construction/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/construction/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/construction/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/construction/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/construction/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/construction/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/construction/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/construction/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/construction" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/construction/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/construction/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/construction/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/construction/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/construction/client")
          return
        }
        setAuthorized(true)
      }
