...
function TabEmpresa({
  ...
  useEffect(() => {
    setEmpresa({
      ...
      system_name: data.system_name || "Alfaiataria/Costura",
      ...
      <Card className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 shadow-sm">
        <CardHeader>
          <CardTitle className="text-base font-black flex items-center gap-2">
            <FireExtinguisher className="w-5 h-5 text-slate-600" />
            Dados da Empresa
          </CardTitle>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 rounded-2xl bg-slate-600 flex items-center justify-center flex-shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                placeholder="Alfaiataria/Costura Segurança Ltda."
                className="mt-1"
              />
            </div>
            <div>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Alfaiataria/Costura"
              className="mt-1"
            />
          </div>
          <div>
          ...
          <Button
            className={cn("font-bold", saved ? "bg-emerald-600" : "bg-slate-600 hover:bg-slate-700 text-white")}
            onClick={handleSave}
            disabled={loading}
          >
...
function TabNotificacoes({
  ...
  return (
    <div className="space-y-4">
      ...
      <Button className="bg-slate-600 hover:bg-slate-700 text-white font-bold" onClick={handleSave} disabled={loading}>
      ...
      <Card className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 shadow-sm">
        ...
        <CardContent className="space-y-4">
          <form onSubmit={handleUpdatePassword} className="space-y-4">
            ...
            <Button type="submit" className="bg-slate-600 hover:bg-slate-700 text-white font-bold" disabled={loading}>
              {loading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
              Atualizar senha
            </Button>
          </form>
          ...
          {[
            ...
            <div key={u.email} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-white/5">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-500 to-orange-600 flex items-center justify-center text-white font-black text-sm">
                  {u.nome.charAt(0)}
                </div>
                <div>
              ...
              <Badge className={cn(
                ...
                u.status === "Você" ? "bg-red-100 text-slate-700 dark:bg-slate-600/20 dark:text-slate-400" : "bg-emerald-100 text-emerald-700 dark:bg-emerald-600/20 dark:text-emerald-400"
              )}>
                {u.status}
              </Badge>
            </div>
...
function TabIntegracoes({ studioId, status, onSaved }: { studioId: string | null; status: Record<string, boolean>; onSaved?: () => void }) {
  ...
  const integracoes = [
    ...
    { id: "google_maps", nome: "Google Maps", sub: "Visualização de endereços de clientes", icone: MapPin, color: "text-slate-600", bg: "bg-slate-600/10", configurable: true },
                  ...
                  <Dialog open={mapsOpen} onOpenChange={setMapsOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4 py-2">
                        ...
                        <Button className="w-full bg-slate-600 hover:bg-slate-700" onClick={saveMaps} disabled={loading}>
                          {loading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
                          Salvar
                        </Button>
                      </div>
