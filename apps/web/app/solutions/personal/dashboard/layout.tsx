...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/personal/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/personal/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/personal/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/personal/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/personal/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/personal/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/personal/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/personal/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/personal/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/personal/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/personal/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/personal/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/personal/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/personal/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/personal/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/personal/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/personal/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
...
function FireSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  const handleLogout = async () => {
    ...
    router.push("/solutions/personal")
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/personal" className="flex items-center gap-2">
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/personal/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/personal/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/personal/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/personal/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/personal/client")
          return
        }
        setAuthorized(true)
      }
