...
export default function FireArchitectDashboard() {
  ...
  const statCards = [
    ...
    { label: "Em Andamento", value: stats.active, color: "text-violet-600", icon: Ruler },
  ...
  const statusColors: Record<string, string> = {
    ...
    in_progress: "bg-red-100 text-emerald-700",
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Link href="/solutions/environmental_compliance/architect/projetos">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/solutions/environmental_compliance/architect/projetos" className="text-sm font-bold text-violet-600 hover:text-violet-700 hover:underline">
            Ver todos
          </Link>
        </div>
        ...
        {projects.length === 0 ? (
          ...
          <div className="grid gap-3">
            {projects.map((project: any) => {
              ...
              return (
                <Link
                  href={isPending ? "/solutions/environmental_compliance/architect/projetos" : `/solutions/environmental_compliance/architect/projetos/${project.id}`}
                  key={project.id}
                >
                  <Card className="hover:shadow-md transition-all cursor-pointer group border-l-4 border-l-violet-600">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-start gap-3">
                        <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0", isPPCI ? "bg-violet-50" : "bg-slate-100")}>
                          {isPPCI ? <Flame className="w-5 h-5 text-violet-600" /> : <Ruler className="w-5 h-5 text-violet-600" />}
                        </div>
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4 className="font-bold text-slate-900 dark:text-white group-hover:text-violet-600 transition-colors">
                              {project.title || `Projeto #${project.id.slice(0, 8)}`}
                            </h4>
                            {isPPCI && <Badge className="bg-violet-100 text-violet-700 text-[10px]">PPCI</Badge>}
                          </div>
