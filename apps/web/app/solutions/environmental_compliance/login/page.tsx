  ...
  React.useEffect(() => {
    async function checkSession() {
      ...
      if (session?.user) {
        ...
        if (userRole === 'engineer') router.push("/solutions/environmental_compliance/engineer")
        else if (userRole === 'architect') router.push("/solutions/environmental_compliance/architect")
        else if (userRole === 'technician' || userRole === 'teacher') router.push("/solutions/environmental_compliance/technician")
        else if (userRole === 'student') router.push("/solutions/environmental_compliance/client")
        else if (userRole === 'super_admin') router.push("/admin")
        else if (userRole === 'finance') router.push("/solutions/environmental_compliance/dashboard/financeiro")
        else router.push("/solutions/environmental_compliance/dashboard")
      }
    }
    checkSession()
  }, [router])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        else if (data.user.role === 'student') router.push("/solutions/environmental_compliance/client")
        else if (data.user.role === 'teacher' || data.user.role === 'technician') router.push("/solutions/environmental_compliance/technician")
        else if (data.user.role === 'engineer') router.push("/solutions/environmental_compliance/engineer")
        else if (data.user.role === 'architect') router.push("/solutions/environmental_compliance/architect")
        else if (data.user.role === 'seller') router.push("/seller")
        else if (data.user.role === 'finance') router.push("/solutions/environmental_compliance/dashboard/financeiro")
        else router.push("/solutions/environmental_compliance/dashboard")
      } else {
        toast({
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-emerald-800 via-emerald-700 to-orange-700 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <Link href="/solutions/environmental_compliance" className="flex items-center gap-3">
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-slate-950 relative overflow-hidden">
        ...
        <div className="relative w-full max-w-md">
          <div className="lg:hidden mb-12 text-center">
            <Link href="/solutions/environmental_compliance" className="inline-flex items-center gap-3">
            ...
            <CardContent className="px-6 pb-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Link href="/forgot-password?returnTo=/solutions/environmental_compliance/login" title="Esqueceu?" className="text-xs text-emerald-500/80 hover:text-emerald-400 transition-colors">
                      Esqueceu?
                    </Link>
                  </div>
                  <div className="relative">
              ...
              <div className="mt-6 pt-5 border-t border-white/5 space-y-4">
                <p className="text-center text-sm text-slate-500">
                  Não possui conta?{" "}
                  <Link href="/solutions/environmental_compliance/register" className="text-emerald-500 hover:text-emerald-400 font-medium transition-colors">
                    Criar conta grátis
                  </Link>
                </p>
                <Link
                ...
                <Link
                  href="/solutions/environmental_compliance"
