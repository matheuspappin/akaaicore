...
function LoginContent() {
  ...
  React.useEffect(() => {
    async function checkSession() {
      ...
      if (session?.user) {
        ...
        if (userRole === 'engineer') router.push("/solutions/brewery/engineer")
        else if (userRole === 'architect') router.push("/solutions/brewery/architect")
        else if (userRole === 'technician' || userRole === 'teacher') router.push("/solutions/brewery/technician")
        else if (userRole === 'student') router.push("/solutions/brewery/client")
        else if (userRole === 'super_admin') router.push("/admin")
        else if (userRole === 'finance') router.push("/solutions/brewery/dashboard/financeiro")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        else if (data.user.role === 'student') router.push("/solutions/brewery/client")
        else if (data.user.role === 'teacher' || data.user.role === 'technician') router.push("/solutions/brewery/technician")
        else if (data.user.role === 'engineer') router.push("/solutions/brewery/engineer")
        else if (data.user.role === 'architect') router.push("/solutions/brewery/architect")
        else if (data.user.role === 'seller') router.push("/seller")
        else if (data.user.role === 'finance') router.push("/solutions/brewery/dashboard/financeiro")
        else router.push("/solutions/brewery/dashboard")
      } else {
        toast({
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-amber-800 via-amber-700 to-orange-700 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-30">
          ...
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-amber-900/50 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2" />
        ...
        <Link href="/solutions/brewery" className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-4">
          <div className="flex -space-x-3">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="w-12 h-12 rounded-full bg-amber-800/50 border-2 border-amber-400/30 backdrop-blur-sm flex items-center justify-center text-white text-xs font-bold"
              >
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-slate-950 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-1/4 w-px h-full bg-gradient-to-b from-transparent via-amber-500/10 to-transparent pointer-events-none" />
        ...
        <div className="relative w-full max-w-md">
          <div className="lg:hidden mb-12 text-center">
            <Link href="/solutions/brewery" className="inline-flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-amber-600 flex items-center justify-center shadow-lg shadow-amber-600/20">
                <FireExtinguisher className="w-6 h-6 text-white" />
              </div>
              <span className="text-2xl font-black text-white tracking-tighter">
                Fire<span className="text-amber-500">Control</span>
              </span>
            </Link>
          </div>
          ...
          <Card className="relative overflow-hidden bg-slate-900/90 backdrop-blur-2xl border border-white/[0.06] shadow-2xl shadow-black/60 ring-1 ring-white/[0.04]">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-px bg-gradient-to-r from-transparent via-amber-500/50 to-transparent" />
            {/* Brilho sutil nas bordas */}
            <div className="absolute -inset-px bg-gradient-to-b from-amber-500/5 via-transparent to-transparent rounded-[inherit] [mask-image:linear-gradient(black,transparent_70%)] pointer-events-none" />
            ...
            <CardContent className="px-6 pb-8">
                        ...
                        {profiles.map((p) => (
                          <button
                            ...
                            className={cn(
                              ...
                              role === p.id ? "bg-amber-500/10 text-amber-400" : "text-slate-400 hover:bg-white/5 hover:text-white"
                            )}
                          >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    required
                    className="bg-slate-800/50 border-white/5 text-white h-11 rounded-xl placeholder:text-slate-500 focus:border-amber-500/40 focus:ring-1 focus:ring-amber-500/20 transition-all"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Link href="/forgot-password?returnTo=/solutions/brewery/login" title="Esqueceu?" className="text-xs text-amber-500/80 hover:text-amber-400 transition-colors">
                      Esqueceu?
                    </Link>
                  </div>
                  <div className="relative">
                    <Input
                      ...
                      required
                      className="bg-slate-800/50 border-white/5 text-white h-11 pr-12 rounded-xl placeholder:text-slate-500 focus:border-amber-500/40 focus:ring-1 focus:ring-amber-500/20 transition-all"
                    />
                    <button
                      type="button"
                ...
                <Button
                  type="submit"
                  className="w-full h-12 rounded-xl bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white font-semibold shadow-lg shadow-amber-500/25 hover:shadow-amber-500/40 transition-all duration-200"
                  disabled={isLoading}
                >
                  {isLoading ? (
                    <>
              ...
              <div className="mt-6 pt-5 border-t border-white/5 space-y-4">
                <p className="text-center text-sm text-slate-500">
                  Não possui conta?{" "}
                  <Link href="/solutions/brewery/register" className="text-amber-500 hover:text-amber-400 font-medium transition-colors">
                    Criar conta grátis
                  </Link>
                </p>
                <Link
