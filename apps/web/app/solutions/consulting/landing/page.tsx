...
import { BarChart,  useState, useEffect } from "react"
...
import { 
  ...
  DollarSign, Layout, 
  ChevronRight, Menu, X, Truck, BarChart,
...
function Navbar() {
  ...
  const navItems = [
    ...
    { name: "App do Consultor", href: "#tech-app" },
  ...
  return (
    <header className={cn(
      ...
      <div className="container mx-auto px-6 flex items-center justify-between">
        <Link href="/solutions/consulting/landing" className="flex items-center gap-2.5 font-bold text-2xl tracking-tight text-white hover:opacity-90 transition-opacity">
          <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-r from-slate-500 via-red-600 to-cyan-500 rounded-xl blur opacity-40 group-hover:opacity-60 transition duration-500"></div>
            <div className="relative w-9 h-9 rounded-xl bg-gradient-to-br from-slate-500 to-red-600 flex items-center justify-center text-white shadow-lg shadow-red-500/30">
          ...
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-cyan-200/90">
            AKAAI <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-red-400 font-black">MKT</span>
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navItems.map((item) => (
            <Link 
              key={item.name} 
              href={item.href} 
              className="px-4 py-2 text-sm font-bold text-white/70 hover:text-slate-400 transition-all relative group uppercase tracking-widest"
            >
              {item.name}
              <span className="absolute bottom-0 left-4 right-4 h-0.5 bg-gradient-to-r from-slate-400 to-cyan-400 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center" />
            </Link>
          ))}
        </nav>

        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/solutions/consulting/login" className="hidden sm:block">
          ...
          <Link href="/solutions/consulting/register">
          ...
          >
            <div className="flex flex-col p-6 space-y-4 font-semibold text-white">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <Link href="/solutions/consulting/login" onClick={() => setMobileMenuOpen(false)}>
                ...
                <Link href="/solutions/consulting/register" onClick={() => setMobileMenuOpen(false)}>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-2xl border border-white/20 bg-white/5 backdrop-blur-xl text-sm font-semibold text-slate-200 mb-10 shadow-[0_0_0_1px_rgba(255,255,255,0.1),0_4px_20px_-4px_rgba(234,88,12,0.2)]"
          >
          ...
          >
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 via-red-500 to-cyan-400">
              AKAAI MKT
            </span>
          </motion.h1>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Tecnologia na palma da mão do <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-red-500">Consultor</span>
          </h2>
              ...
              <div 
                ...
                className={cn(
                  ...
                  activeTab === feature.id 
                    ? "border-slate-500/40 bg-slate-500/5 shadow-lg shadow-slate-500/10" 
                    : "border-white/10 bg-white/5 hover:bg-white/[0.07] hover:border-slate-500/20",
                  feature.id === 'vistoria' && "animate-pulse group-hover:animate-shake"
                )}
              >
                <div className="flex gap-4">
                  <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", activeTab === feature.id ? "bg-gradient-to-br from-slate-500 to-red-600 text-white shadow-lg shadow-slate-500/30" : "bg-slate-700/80 text-slate-400")}>
...
function ArchitectPortalMockup() {
  return (
    <section id="architect-portal" className="py-24 text-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <Badge className="bg-slate-500/20 text-slate-400 border border-slate-500/30 px-5 py-2 mb-4 rounded-2xl uppercase tracking-widest text-[10px] font-black shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">Portal do Arquiteto</Badge>
          <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white">
            Agilidade para seus <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-amber-500">Projetos PPCI</span>
          </h2>
          <p className="text-slate-300 max-w-2xl mx-auto text-lg">
          ...
          <div className="md:col-span-7 space-y-6 order-2 md:order-1">
            {[
              ...
              <motion.div 
                key={feature.id}
                whileHover={{ x: 10 }}
                className="p-6 rounded-2xl transition-all border border-slate-500/25 bg-slate-500/5 backdrop-blur-xl hover:border-slate-400/50 hover:bg-slate-500/10 shadow-lg shadow-slate-500/10 group"
              >
                <div className="flex gap-4">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-slate-600 text-white shadow-lg shadow-slate-600/20 group-hover:scale-110 transition-transform">
                    <feature.icon className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-white">{feature.title}</h3>
                ...
                <div className="flex flex-col h-full">
                  <div className="bg-slate-950 p-4 rounded-2xl shadow-2xl border border-slate-800 overflow-hidden mb-4">
                    <div className="bg-slate-800 p-3 flex items-center justify-between border-b border-slate-700">
                        ...
                        <div className="w-7 h-7 rounded-full bg-slate-600 flex items-center justify-center font-bold text-[10px] text-white">ARQ</div>
                    </div>
                    <div className="p-5 bg-slate-900 space-y-4">
                        <div className="bg-slate-800/20 border border-slate-700/50 p-3 rounded-xl">
                            <div className="flex items-center gap-2 mb-1">
                                <AlertTriangle className="w-4 h-4 text-slate-500 shrink-0" />
                                <p className="text-[11px] font-bold text-slate-300 uppercase tracking-tighter">Aprovação Pendente</p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-slate-800 p-3 rounded-xl text-center border border-white/5">
                                <p className="text-2xl font-black text-slate-400">12</p>
                        ...
                        <div className="space-y-2">
                          <Button variant="outline" size="sm" className="w-full bg-slate-600 hover:bg-slate-500 border-none text-white text-[10px] font-black h-9 rounded-xl uppercase tracking-widest shadow-[0_4px_12px_rgba(234,88,12,0.3)] hover:shadow-[0_6px_16px_rgba(234,88,12,0.4)] transition-all">
...
function AdminDashboardMockup() {
  const modules = [
    ...
    { label: 'Estoque', icon: Box, color: "bg-slate-500", shadow: "shadow-slate-500/20", description: "Controle de carga de extintores (PQS, CO2) e peças de reposição." },
        ...
        <div className="container mx-auto px-6">
            ...
            <div className="max-w-6xl mx-auto rounded-[2.5rem] shadow-2xl overflow-hidden relative backdrop-blur-xl bg-slate-900/60 border border-white/10">
                ...
                <div className="p-8 bg-white/5 rounded-b-[2.5rem]">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
                        {[
                            ...
                            { label: "Novos Clientes", value: "6", trend: "+2 hoje", color: "text-slate-500", icon: UserPlus },
                        ].map((stat, i) => (
                            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="p-6 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-sm hover:shadow-md hover:border-slate-500/20 transition-all">
                ...
                <div className="absolute bottom-0 right-0 p-8 opacity-5 pointer-events-none">
                    <BarChart className="w-64 h-64 rotate-12" />
                </div>
            </div>
        </div>
    </section>
...
function FeatureGrid() {
  const features = [
    ...
    { title: "Alertas CRM", description: "Aviso automático via WhatsApp 30 dias antes.", icon: AlertTriangle, color: "from-slate-500 to-amber-400" },
        ...
        <div className="text-center mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white mb-6 tracking-tight">
              Tudo em um <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 via-red-500 to-cyan-400">só lugar</span>
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {features.map((feature, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ y: -10 }} className="group rounded-[2.5rem] border border-white/10 p-8 shadow-sm hover:shadow-2xl transition-all duration-500 relative overflow-hidden backdrop-blur-xl bg-white/[0.03] hover:bg-white/[0.06] hover:border-slate-500/20 hover:shadow-slate-500/10">
              <div className="absolute -inset-px rounded-[2.5rem] bg-gradient-to-br from-slate-500/0 via-transparent to-cyan-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="relative z-10">
                ...
                <div className="mt-8 flex items-center text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-cyan-400 opacity-0 group-hover:opacity-100 transform translate-x-[-10px] group-hover:translate-x-0 transition-all duration-300">
    ...
    <section id="pricing" className="py-24 text-white relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10 text-center max-w-4xl">
        ...
        <div className="grid sm:grid-cols-2 gap-6 mb-16">
          {highlights.map((h, i) => (
            <div key={i} className="flex items-start gap-4 rounded-2xl p-6 border border-white/10 bg-white/5 backdrop-blur-xl hover:border-slate-500/20 hover:bg-white/[0.07] text-left transition-all">
        ...
        <div className="relative rounded-3xl p-10 overflow-hidden backdrop-blur-xl border border-slate-500/30 bg-gradient-to-br from-slate-600/20 via-red-600/15 to-cyan-600/15">
          ...
          <div className="relative">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
              <a href="https://wa.me/5511999999999?text=Olá! Tenho interesse no FireControl para minha empresa de extintores." target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 bg-white hover:bg-white/95 text-slate-900 font-bold h-14 px-8 rounded-2xl transition-all duration-200 shadow-[0_0_0_1px_rgba(255,255,255,0.2),0_12px_40px_-8px_rgba(0,0,0,0.4)] hover:shadow-[0_0_0_1px_rgba(255,255,255,0.3),0_16px_50px_-8px_rgba(239,68,68,0.2)] hover:scale-[1.02] group">
                <BarChart className="w-5 h-5 group-hover:rotate-[-8deg] transition-transform" />
  ...
  return (
    <footer className="text-slate-400 py-12 border-t border-white/10 text-center backdrop-blur-sm">
      <div className="flex items-center justify-center gap-2 mb-6 font-bold text-xl text-white">
        ...
        <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 via-red-500 to-cyan-400">AKAAI MKT</span>
      </div>
