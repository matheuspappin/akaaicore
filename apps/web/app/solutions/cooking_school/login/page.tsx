...
function LoginContent() {
  ...
  React.useEffect(() => {
    async function checkSession() {
      ...
      if (session?.user) {
        ...
        if (userRole === 'engineer') router.push("/solutions/cooking_school/engineer")
        else if (userRole === 'architect') router.push("/solutions/cooking_school/architect")
        else if (userRole === 'technician' || userRole === 'teacher') router.push("/solutions/cooking_school/technician")
        else if (userRole === 'student') router.push("/solutions/cooking_school/client")
        else if (userRole === 'super_admin') router.push("/admin")
        else if (userRole === 'finance') router.push("/solutions/cooking_school/dashboard/financeiro")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        else if (data.user.role === 'student') router.push("/solutions/cooking_school/client")
        else if (data.user.role === 'teacher' || data.user.role === 'technician') router.push("/solutions/cooking_school/technician")
        else if (data.user.role === 'engineer') router.push("/solutions/cooking_school/engineer")
        else if (data.user.role === 'architect') router.push("/solutions/cooking_school/architect")
        else if (data.user.role === 'seller') router.push("/seller")
        else if (data.user.role === 'finance') router.push("/solutions/cooking_school/dashboard/financeiro")
        else router.push("/solutions/cooking_school/dashboard")
      } else {
        toast({
  ...
  const handleBackToLanding = () => {
    router.push("/solutions/cooking_school")
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex relative font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-orange-800 via-orange-700 to-orange-700 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-30">
          ...
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-orange-900/50 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2" />
        ...
        <Link href="/solutions/cooking_school" className="flex items-center gap-3">
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-slate-950 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-1/4 w-px h-full bg-gradient-to-b from-transparent via-orange-500/10 to-transparent pointer-events-none" />
        ...
        <div className="relative w-full max-w-md">
          <div className="lg:hidden mb-12 text-center">
            <Link href="/solutions/cooking_school" className="inline-flex items-center gap-3">
          ...
          <Card className="relative overflow-hidden bg-slate-900/90 backdrop-blur-2xl border border-white/[0.06] shadow-2xl shadow-black/60 ring-1 ring-white/[0.04]">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-px bg-gradient-to-r from-transparent via-orange-500/50 to-transparent" />
            {/* Brilho sutil nas bordas */}
            <div className="absolute -inset-px bg-gradient-to-b from-orange-500/5 via-transparent to-transparent rounded-[inherit] [mask-image:linear-gradient(black,transparent_70%)] pointer-events-none" />
            ...
            <CardContent className="px-6 pb-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Link href="/forgot-password?returnTo=/solutions/cooking_school/login" title="Esqueceu?" className="text-xs text-orange-500/80 hover:text-orange-400 transition-colors">
                      Esqueceu?
                    </Link>
                  </div>
                  <div className="relative">
              ...
              <div className="mt-6 pt-5 border-t border-white/5 space-y-4">
                <p className="text-center text-sm text-slate-500">
                  Não possui conta?{" "}
                  <Link href="/solutions/cooking_school/register" className="text-orange-500 hover:text-orange-400 font-medium transition-colors">
                    Criar conta grátis
                  </Link>
                </p>
                <Link
