...
export default function FireEngineerDashboard() {
  ...
  const statCards = [
    ...
    { label: "Em Andamento", value: stats.active, color: "text-orange-600", icon: HardHat },
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Link href="/solutions/cooking_school/engineer/projetos">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/solutions/cooking_school/engineer/projetos" className="text-sm font-bold text-orange-600 hover:text-orange-700 hover:underline">
            Ver todos
          </Link>
        </div>
        ...
        {projects.length === 0 ? (
          ...
          <div className="grid gap-3">
            {projects.map((project: any) => {
              ...
              return (
                <Link
                  href={isPending ? "/solutions/cooking_school/engineer/projetos" : `/solutions/cooking_school/engineer/projetos/${project.id}`}
                  key={project.id}
                >
                  <Card className="hover:shadow-md transition-all cursor-pointer group border-l-4 border-l-orange-600">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-start gap-3">
                        <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0", isPPCI ? "bg-red-50" : "bg-orange-50")}>
                          {isPPCI ? <Flame className="w-5 h-5 text-orange-600" /> : <HardHat className="w-5 h-5 text-orange-600" />}
                        </div>
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4 className="font-bold text-slate-900 dark:text-white group-hover:text-orange-600 transition-colors">
                              {project.title || `Projeto #${project.id.slice(0, 8)}`}
                            </h4>
                            {isPPCI && <Badge className="bg-red-100 text-orange-700 text-[10px]">PPCI</Badge>}
                          </div>
