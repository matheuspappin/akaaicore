...
const navGroups = [
  {
    label: "Operacional",
    items: [
      { href: "/solutions/cooking_school/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { href: "/solutions/cooking_school/dashboard/clientes", label: "Clientes / Edificações", icon: Building2 },
      { href: "/solutions/cooking_school/dashboard/tecnicos", label: "Técnicos", icon: Wrench },
      { href: "/solutions/cooking_school/dashboard/engenheiros", label: "Engenheiros", icon: HardHat },
      { href: "/solutions/cooking_school/dashboard/arquitetos", label: "Arquitetos", icon: Ruler },
      { href: "/solutions/cooking_school/dashboard/os", label: "Ordens de Serviço", icon: ClipboardList },
      { href: "/solutions/cooking_school/dashboard/extintores", label: "Extintores", icon: Package },
      { href: "/solutions/cooking_school/dashboard/vistorias", label: "Vistorias", icon: Calendar },
    ],
  },
  {
    label: "Comercial",
    items: [
      { href: "/solutions/cooking_school/dashboard/vendas", label: "PDV — Vendas", icon: ShoppingCart },
      { href: "/solutions/cooking_school/dashboard/portal-vendedor", label: "Portal do Vendedor", icon: UserCheck },
      { href: "/solutions/cooking_school/dashboard/leads", label: "Leads / CRM", icon: TrendingUp },
      { href: "/solutions/cooking_school/dashboard/whatsapp", label: "WhatsApp", icon: Phone },
      { href: "/solutions/cooking_school/dashboard/chat", label: "Chat IA", icon: MessageSquare },
    ],
  },
  {
    label: "Gestão",
    items: [
      { href: "/solutions/cooking_school/dashboard/financeiro", label: "Financeiro", icon: DollarSign },
      { href: "/solutions/cooking_school/dashboard/planos", label: "Planos e Assinatura", icon: Package },
      { href: "/solutions/cooking_school/dashboard/relatorios", label: "Relatórios", icon: BarChart3 },
      { href: "/solutions/cooking_school/dashboard/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
]
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        {!collapsed && (
          <Link href="/solutions/cooking_school" className="flex items-center gap-2">
            ...
            <span className="font-black text-white tracking-tighter text-sm">
              Fire<span className="text-orange-400">Control</span>
            </span>
          </Link>
        )}
        {collapsed && (
...
export default function FireDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        router.push("/solutions/cooking_school/login")
      } else {
        ...
        if (role === 'engineer') {
          router.push("/solutions/cooking_school/engineer")
          return
        }
        if (role === 'architect') {
          router.push("/solutions/cooking_school/architect")
          return
        }
        if (role === 'technician' || role === 'teacher') {
          router.push("/solutions/cooking_school/technician")
          return
        }
        if (role === 'student') {
          router.push("/solutions/cooking_school/client")
          return
        }
        setAuthorized(true)
      }
