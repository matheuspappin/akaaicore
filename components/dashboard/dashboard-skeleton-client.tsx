    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-background">
        ...
        <main className="p-6 space-y-8 animate-in fade-in duration-500">
           <div className="h-8 bg-slate-100 dark:bg-zinc-900 rounded animate-pulse w-48 mb-6" />
           
           <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[1,2,3,4].map(i => (
                <div key={i} className="h-32 bg-slate-100 dark:bg-zinc-900 rounded-xl animate-pulse" />
              ))}
           </div>

           <div className="h-64 bg-slate-100 dark:bg-zinc-900 rounded-xl animate-pulse" />
        </main>
      </div>
    </div>
  )
